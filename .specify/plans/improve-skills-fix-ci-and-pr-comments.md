...
─────────────────────────────────    ────────────────────────────────────────
.specify/core/commands/              .claude/commands/   (git-tracked, regenerated)
.specify/oe/commands/          ──►   .cursor/commands/   (git-tracked, regenerated)
.ai/skills/<name>/commands/
```
...
Currently nothing in AGENTS.md documents that `.claude/commands/` and
`.cursor/commands/` are generated outputs — a contributor who edits them
directly will have their changes silently overwritten next time someone runs the
install script.
...
6. Commit source files + generated outputs in one commit
