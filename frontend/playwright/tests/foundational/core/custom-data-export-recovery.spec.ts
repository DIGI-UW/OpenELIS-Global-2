...
for (const viewport of [
  ...
  test(`a naturally queued export requires confirmation and stays cancelled at ${viewport.width}px`, async ({
    ...
    // The measured public workload took 218 seconds. This is an observation
    // window for the 50,000-result qualification, not a response-time guarantee.
    ...
    await expect(link).toBeVisible({ timeout: 360_000 });
    const downloading = page.waitForEvent("download");
    await link.click();
    const download = await downloading;
    expect(await download.failure()).toBeNull();
    await download.saveAs(testInfo.outputPath("workload.csv"));
    const stream = await download.createReadStream();
    const chunks: Buffer[] = [];
    for await (const chunk of stream) chunks.push(Buffer.from(chunk));
    const bytes = Buffer.concat(chunks);
    ...
    const evidencePath = testInfo.outputPath("cancellation.json");
    await writeFile(
      evidencePath,
      JSON.stringify(
