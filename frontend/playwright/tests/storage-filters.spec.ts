...
test.describe("Storage filters parity migration", () => {
  ...
  test("tab-specific filter controls render across storage tabs", async ({
    ...
    await expect(page.getByTestId("location-filter-dropdown")).toBeVisible();
    ...
    await expect(page.locator("#filter-room")).toBeVisible();
    await expect(page.locator("#filter-status")).toBeVisible();

    await page.getByTestId("tab-shelves").click();
    await expect(page.locator("#filter-room")).toBeVisible();
    await expect(page.locator("#filter-device")).toBeVisible();
    await expect(page.locator("#filter-status")).toBeVisible();
