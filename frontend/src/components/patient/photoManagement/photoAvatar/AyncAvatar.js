 ...
 * 
 */
const AsyncAvatar = ({ patientId, hasPhoto, patientName = 'Patient', size = 40 , gender}) => {
  ...
  if (!hasPhoto) {
    return (
      <div className="async-avatar-placeholder" style={{ width: size, height: size }}>
  ...
  if (loading) {
    return (
      <div className="async-avatar-skeleton" style={{ width: size, height: size }}>
        <SkeletonPlaceholder style={{ width: size, height: size, borderRadius: '50%' }} />
      </div>
    );
  }
  ...
  return (
    <div className="async-avatar-container" style={{ width: size, height: size }}>
      <img
