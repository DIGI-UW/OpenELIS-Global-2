...
const PatientEditByProjectForm = ({ formData, projectData }) => {
  if (!formData) {
  ...
  // Use projectData instead of formData
  useEffect(() => {
    if (formData) {
      setData(formData); // Populate form with projectData
    }
  }, [formData]);

  // Call API using the requestBody
  ...
  const formattedPriorDiseasesList = formattedObservations(
    observations.priorDiseasesList,
  );
  const formattedCurrentDiseasesList = formattedObservations(
    observations.currentDiseasesList,
  );
  const formattedRtnPriorDiseasesList = formattedObservations(
    observations.rtnPriorDiseasesList,
  );
  const formattedRtnCurrentDiseasesList = formattedObservations(
    observations.rtnCurrentDiseasesList,
  );
