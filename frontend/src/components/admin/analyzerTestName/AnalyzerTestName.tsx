...
interface AnalyzerTestMenuItem {
  uniqueId: string;
  analyzerName: string;
...
interface AnalyzerTestMenuResponse {
  menuList: AnalyzerTestMenuItem[];
...
interface AnalyzerTestTableRow {
  id: string;
  analyzerName: string;
  actualTestName: string;
  analyzerTestName: string;
  analyzerNameOnly: string;
  disabled?: boolean;
}
...
function AnalyzerTestName() {
  ...
  const { reloadConfiguration } = useContext(
  ...
  const [AnalyzerTestName, setAnalyzerTestName] =
    useState<AnalyzerTestMenuResponse>({} as AnalyzerTestMenuResponse);
  const [AnalyzerTestNameShow, setAnalyzerTestNameShow] = useState<
    AnalyzerTestTableRow[]
  >([]);
  ...
  const [analyzerList, setAnalyzerList] = useState<AnalyzerOption[]>([]);
  const [testList, setTestList] = useState<TestOption[]>([]);
  const [selectedAnalyzer, setSelectedAnalyzer] =
    useState<AnalyzerOption | null>(null);
  const [selectedAnalyzerId, setSelectedAnalyzerId] = useState<string | null>(
    null,
  );
  ...
  const [originalAnalyzerId, setOriginalAnalyzerId] = useState<string | null>(
    null,
  );
  ...
  const handleNextPage = () => {
    setPaging((pager) => Math.max(pager as number, 2));
    setStartingRecNo(fromRecordCount);
  };

  const handlePreviousPage = () => {
    setPaging((pager) => Math.max((pager as number) - 1, 1));
  ...
  return (
    <>
      ...
      <div className="adminPageContent">
        ...
        >
          {/* @ts-expect-error -- Carbon requires a label that the original JSX did not pass */}
          <Dropdown
          ...
          <>
            <Grid fullWidth={true} className="gridBoundary">
              <Column lg={4} md={8} sm={4}>
                {/* @ts-expect-error -- preserve the original Select without adding a runtime id */}
                <Select
                  labelText={intl.formatMessage({
