...
interface TestNotificationMenuItem {
  id?: string;
  testId: string;
  patientEmail: { active: boolean };
  patientSMS: { active: boolean };
  providerEmail: { active: boolean };
  providerSMS: { active: boolean };
}

interface TestNotificationMenuData {
  formMethod?: string;
  cancelAction?: string;
  submitOnCancel?: string;
  cancelMethod?: string;
  adminMenuItems?: unknown[];
  totalRecordCount?: string;
  fromRecordCount?: string;
  toRecordCount?: string;
  selectedIDs?: string[];
  menuList: TestNotificationMenuItem[];
}
...
function TestNotificationConfigMenu() {
  ...
  const [testNamesList, setTestNamesList] = useState<TestNameOption[]>([]);
  const [testNotificationConfigMenuData, setTestNotificationConfigMenuData] =
    useState<TestNotificationMenuData>({} as TestNotificationMenuData);
  const [
    testNotificationConfigMenuDataPost,
    setTestNotificationConfigMenuDataPost,
  ] = useState<TestNotificationMenuData>({ menuList: [] });
