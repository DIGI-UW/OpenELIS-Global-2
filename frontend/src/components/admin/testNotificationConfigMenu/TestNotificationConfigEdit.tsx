...
interface PayloadTemplate {
  ...
  [key: string]: string | PayloadTemplate | undefined;
}

interface NotificationChannelConfig {
  active: boolean;
  [key: string]: boolean | PayloadTemplate;
}

interface TestNotificationConfig {
  testId?: string;
  providerEmail: NotificationChannelConfig;
  providerSMS: NotificationChannelConfig;
  patientEmail: NotificationChannelConfig;
  patientSMS: NotificationChannelConfig;
}

interface TestNotificationEditData {
  formName?: string;
  formMethod?: string;
  cancelAction?: string;
  submitOnCancel?: string;
  cancelMethod?: string;
  config?: TestNotificationConfig;
  systemDefaultPayloadTemplate?: PayloadTemplate;
  editSystemDefaultPayloadTemplate?: boolean;
}
...
function TestNotificationConfigEdit() {
  ...
  const [testNotificationConfigEditData, setTestNotificationConfigEditData] =
    useState<TestNotificationEditData>({});
  const [
  ...
  const setTemplateField = (templatePath, field) => (e) => {
    ...
    setTestNotificationConfigEditDataPost((prev) => {
      const next = { ...prev } as TestNotificationEditData & PayloadTemplate;
      const parts = templatePath.split(".");
      let parent: PayloadTemplate = next;
      for (let i = 0; i < parts.length - 1; i++) {
        parent[parts[i]] = { ...((parent[parts[i]] || {}) as PayloadTemplate) };
        parent = parent[parts[i]] as PayloadTemplate;
      }
      const lastKey = parts[parts.length - 1];
      // prettier-ignore
      parent[lastKey] = { ...((parent[lastKey] || {}) as PayloadTemplate), [field]: value } as PayloadTemplate;
      return next;
    });
  };

  const readTemplateField = (
    templatePath: string,
    field: "subjectTemplate" | "messageTemplate",
  ) => {
    const parts = templatePath.split(".");
    let cursor: TestNotificationEditData | PayloadTemplate | undefined =
      testNotificationConfigEditDataPost;
    for (const p of parts) {
      if (cursor == null) return "";
      cursor = (cursor as PayloadTemplate)[p] as PayloadTemplate | undefined;
    }
    return (cursor as PayloadTemplate | undefined)?.[field] ?? "";
  };
          ...
          <div>
            ...
            <Grid fullWidth={true}>
              <Column lg={16} md={8} sm={4}>
                <TextArea
                  id="message"
                  // @ts-expect-error -- preserve the original TextArea type prop
                  type="text"
                  labelText=""
                  hideLabel={true}
                  disabled={sysDefaultMsg}
