...
interface ConfigurationContextValue {
  configurationProperties: {
    restrictFreeTextProviderEntry?: string;
  };
}
...
interface TestSectionOption {
  id: string;
  value: string;
}

type TestSectionRoleMap = Record<string, string[]>;

interface UserData {
  accountActive?: string;
  accountDisabled?: string;
  accountLocked?: string;
  allowCopyUserRoles?: string;
  cancelAction?: string;
  cancelMethod?: string;
  ...
  labUnitRoles?: UserRole[];
  loginUserId?: string;
  selectedRoles?: string[];
  selectedTestSectionLabUnits?: TestSectionRoleMap;
  systemUserId?: string;
  systemUserIdToCopy?: string;
  systemUserLastupdated?: string;
  testSections?: TestSectionOption[];
  timeout?: string;
...
function UserAddModify() {
  ...
  const [copyUserPermissionList, setCopyUserPermissionList] = useState<
    unknown[] | null
  >(null);
  const [userData, setUserData] = useState<UserData | null>(null);
  const [userDataShow, setUserDataShow] = useState<Partial<UserData>>({});
  const [userDataPost, setUserDataPost] = useState<Partial<UserData> | null>(
    null,
  );
  ...
  // eslint-disable-next-line local/no-useeffect-timer-leaks -- preserve the legacy redirect timing during this type-only migration
  useEffect(() => {
  ...
  const handleUserData = (res) => {
    if (!res) {
      ...
      // eslint-disable-next-line no-var -- preserve the original JavaScript var declaration
      var KeyList: string[] = [];
  ...
  function handleCopyUserPermissionsChangeClick() {
    setSelectedTestSectionLabUnits([] as unknown as TestSectionRoleMap);
    setSelectedTestSectionList([]);
    userSavePostCall();
  }
