...
interface TestSectionAssignmentData {
  sectionTestList?: Record<string, AssignmentTest[]>;
  testSectionList?: Array<{ id: string; value: string }>;
}

interface TestSectionAssignmentPayload {
  testSectionIdNew?: string;
  testSectionNameNew?: string;
  testId?: string;
  testValue?: string;
  testSectionNameOld?: string;
  testSectionIdOld?: string;
}
...
function TestSectionTestAssign() {
  ...
  const [testSectionTestAssignModal, setTestSectionTestAssignModal] =
    useState(false);
  const [testSectionTestAssign, setTestSectionTestAssign] =
    useState<TestSectionAssignmentData>({});
  const [testSectionTestAssignPost, setTestSectionTestAssignPost] =
    useState<TestSectionAssignmentPayload>({
      testSectionIdNew: "",
      testSectionNameNew: "",
      testId: "",
      testValue: "",
                      ...
                      <React.Fragment key={`${sectionKey}-${sectionId}`}>
                        ...
                        {tests.map((test) => (
                          ...
                          >
                            <ClickableTile
                              onClick={() => {
                                // eslint-disable-next-line @typescript-eslint/no-unused-expressions -- preserve the original comma expression
