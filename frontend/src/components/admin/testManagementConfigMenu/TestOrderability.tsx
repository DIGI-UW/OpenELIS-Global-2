...
interface OrderabilityTest {
  id: string;
  value: string;
}

interface OrderabilitySampleGroup {
  sampleType: { id: string; value: string };
  activeTests: OrderabilityTest[];
  inactiveTests: OrderabilityTest[];
}

interface TestOrderabilityData {
  orderableTestList: OrderabilitySampleGroup[];
}
...
function TestOrderability() {
  ...
  const [testOrderabilityData, setTestOrderabilityData] =
    useState<TestOrderabilityData>({} as TestOrderabilityData);
                    ...
                    return (
                      ...
                      >
                        <Checkbox
                          ...
                          checked={isActive}
                          onChange={(_, { checked }) => {
                            // eslint-disable-next-line @typescript-eslint/no-unused-expressions -- preserve the original ternary expression
                            checked
      ...
      >
        <Grid fullWidth={true}>
          <Column lg={16} md={8} sm={4}>
            ...
            <Section>
              {(() => {
                const groupedInactiveTests: Record<string, string[]> = {};
