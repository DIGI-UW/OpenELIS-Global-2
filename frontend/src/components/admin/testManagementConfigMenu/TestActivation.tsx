...
interface ActivationSampleType {
  id: string;
  value?: string;
}

interface ActivationSampleGroup {
  sampleType: ActivationSampleType;
  activeTests: ActivationTest[];
  inactiveTests: ActivationTest[];
  activateTest?: ActivationTest[];
  inactiveTest?: ActivationTest[];
}

interface TestActivationData {
  activeTestList: ActivationSampleGroup[];
  inactiveTestList: ActivationSampleGroup[];
}
...
function TestActivation() {
  ...
  const [testActivationData, setTestActivationData] =
    useState<TestActivationData>({} as TestActivationData);
  const [changedTestActivationData, setChangedTestActivationData] =
    useState<TestActivationData>({} as TestActivationData);
    ...
    setJsonChangeList((prev) => {
      ...
      if (isChecked === true) {
        if (!isOriginallyActive) {
          if (activateTest.some((item) => item.id === test.id)) {
            (
              handleActiveTestsOnChangeSetJsonChangeListRemove as unknown as (
                test: ActivationTest,
    ...
    setJsonChangeList((prev) => {
      ...
      if (isChecked === false) {
        if (!isOriginallyInactive) {
          ...
          if (
            sample?.activeTests?.length > 0 &&
            (sample?.inactiveTests as unknown) === 1
          ) {
            if (!deactivateSample.some((item) => item.id === sampleTypeId)) {
