...
interface TestModifyFiltersProps {
  sampleTypeList: FilterOption[];
  labUnitList: FilterOption[];
  selectedSampleType: string;
  selectedTestSection: string;
  onFilterChange: (sampleType: string, testSection: string) => void;
  onClearFilters?: () => void;
}
...
function TestModifyFilters({
  ...
  const handleTestSectionChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    const newValue = e.target.value;
    onFilterChange("", newValue);
  };
