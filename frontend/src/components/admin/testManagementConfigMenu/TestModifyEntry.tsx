...
interface TestFilterOption {
  id: string;
  value: string;
}

interface TestCatalogBean {
  id: string;
  localization?: { english?: string; french?: string };
  [key: string]: unknown;
}

interface TestModifyResponse {
  testCatBeanList: TestCatalogBean[];
  sampleTypeList: TestFilterOption[];
  labUnitList: TestFilterOption[];
}
...
function TestModifyEntry() {
  ...
  const [testMonifyList, setTestModifyList] = useState<TestModifyResponse>(
    {} as TestModifyResponse,
  );
  const [filteredTests, setFilteredTests] = useState<TestFilterOption[]>([]);
  const [searchFilteredTests, setSearchFilteredTests] = useState<
    TestFilterOption[]
  >([]);
  const [showGuide, setShowGuide] = useState(false);
  const [selectedTestIdToEdit, setSelectedTestIdToEdit] = useState<
    string | null
  >(null);
        ...
        <div className="orderLegendBody">
          ...
          <Grid fullWidth={true}>
            ...
            <Column lg={16} md={8} sm={4}>
              <Toggle
                id="toggle"
                // @ts-expect-error -- preserve the original FormattedMessage node
