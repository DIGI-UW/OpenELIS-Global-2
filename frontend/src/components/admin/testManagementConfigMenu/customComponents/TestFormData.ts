export interface TestDictionaryValue {
  id?: string;
  value?: string;
  qualified?: string;
}

export interface TestResultLimitForm {
  ageRange: string;
  highAgeRange: string;
  gender: boolean;
  lowNormal: string;
  highNormal: string;
  lowNormalFemale?: string;
  highNormalFemale?: string;
}

export interface TestFormValues {
  testId?: string;
  testNameEnglish: string;
  testNameFrench: string;
  testReportNameEnglish: string;
  testReportNameFrench: string;
  testSection: string;
  panels: Array<string | { id?: string; value?: string }>;
  uom: string;
  loinc: string;
  resultType: string;
  orderable: string;
  ...
  active: string;
  dictionary: TestDictionaryValue[];
  dictionaryReference: string;
  defaultTestResult: string;
  sampleTypes: string[];
  lowValid: string;
  highValid: string;
  lowReportingRange: string;
  highReportingRange: string;
  lowCritical: string;
  highCritical: string;
  significantDigits: string;
  resultLimits: TestResultLimitForm[];
}

export const TestFormData: TestFormValues = {
  testNameEnglish: "",
  testNameFrench: "",
...
export const extractAgeRangeParts = (rangeStr?: string) => {
...
const isNumericRange = (str: unknown): str is string => {
...
const extractRange = (rangeStr: unknown): [string, string] => {
    ...
    resultLimits:
      (test.resultLimits || []).length === 0
        ...
        : Object.entries(
            ...
            (
              limits as Array<{ normalRange?: string; gender?: string }>
            ).forEach((limit) => {
              let low = "-Infinity",
                high = "Infinity";
