...
function SampleTypeActivation() {
  ...
  const loadSampleTypes = () => {
    setIsLoading(true);
    getFromOpenElisServer(`/rest/SampleTypeManagement`, (res) => {
      ...
      (res) => {
        if (res && res.success) {
          ...
          addNotification({
            ...
            title: intl.formatMessage({ id: "notification.title" }),
            message:
              res?.message || intl.formatMessage({ id: "server.error.msg" }),
          });
          setNotificationVisible(true);
        }
  ...
  const rows = [
    ...
    },
    {
      ...
      description: (
        <FormattedMessage id="configuration.sampleType.activate.explain" />
      ),
    },
  ];
  ...
  const headers = [
    ...
    {
      key: "testCount",
      header: intl.formatMessage({ id: "sample.type.test.count" }),
    },
    {
      key: "storageDefaults",
                  ...
                  render={({
                    ...
                    >
                      ...
                      >
                        ...
                        <TableBody>
                          {rows.map((row) => {
                            const sampleType = sampleTypeList.find(
                              (st) => st.id.toString() === row.id,
                            );
                            const currentStatus =
                              getSampleTypeStatus(sampleType);
                            const hasTests = (sampleType?.testCount || 0) > 0;
