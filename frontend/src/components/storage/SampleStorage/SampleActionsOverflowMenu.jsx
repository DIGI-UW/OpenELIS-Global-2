...
const SampleActionsOverflowMenu = ({
  ...
  const handleMove = useCallback((event) => {
    console.log("SampleActionsOverflowMenu: handleMove called", { sample, event, hasOnMove: !!onMove });
    ...
    if (onMove) {
      ...
      try {
        ...
        console.error("SampleActionsOverflowMenu: error in onMove callback", error);
      }
    } else {
      console.warn("SampleActionsOverflowMenu: onMove callback not provided for sample", sample?.sampleId);
    }
  }, [sample, onMove]);

  const handleDispose = useCallback((event) => {
    console.log("SampleActionsOverflowMenu: handleDispose called", { sample, event, hasOnDispose: !!onDispose });
    if (event) {
      event.preventDefault?.();
      event.stopPropagation?.();
    }
    if (onDispose) {
      ...
      try {
        ...
        console.error("SampleActionsOverflowMenu: error in onDispose callback", error);
      }
    } else {
      console.warn("SampleActionsOverflowMenu: onDispose callback not provided for sample", sample?.sampleId);
    }
  }, [sample, onDispose]);

  const handleViewStorage = useCallback((event) => {
    console.log("SampleActionsOverflowMenu: handleViewStorage called", { sample, event, hasOnViewStorage: !!onViewStorage });
    if (event) {
      event.preventDefault?.();
      event.stopPropagation?.();
    }
    if (onViewStorage) {
      console.log("SampleActionsOverflowMenu: executing onViewStorage callback");
      try {
        onViewStorage(sample);
      } catch (error) {
        console.error("SampleActionsOverflowMenu: error in onViewStorage callback", error);
      }
    } else {
      console.warn("SampleActionsOverflowMenu: onViewStorage callback not provided for sample", sample?.sampleId);
    }
