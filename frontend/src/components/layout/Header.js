...
function OEHeader(props) {
  ...
  const [menuLoadKey, setMenuLoadKey] = useState(0);
  ...
  const handleMenuItems = (tag, res) => {
    if (res) {
      ...
      // Increment key to force SideNav re-render when menu data loads
      if (tag === "menu") {
        setMenuLoadKey((prev) => prev + 1);
      }
    }
  };
