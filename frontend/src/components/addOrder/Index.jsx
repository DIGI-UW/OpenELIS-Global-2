...
const Index = () => {
  ...
  /**
   * Re-reads the order that was just saved so tests generated by reflex or
   * calculated rules appear. A read, not a browser reload: the page keeps its
   * state and simply learns what the order became.
   */
  const refreshSavedOrder = (labNo) => {
    if (!labNo) {
      return;
    }
    getLabOrder(
      ...
      (order) => {
        if (order) {
          setSaveResponse((previous) => ({ ...previous, ...order }));
        }
      },
      () => {
        // The order saved; failing to re-read it only costs the refresh.
      },
    );
  };
  ...
  const handlePost = (response) => {
    ...
    if (response && !response.error && responseStatus < 400) {
      ...
      setPage(page + 1);
      // Saving the order is what runs the reflex and calculated rules, so the
      // tests they generate exist only on the server's copy - the response
      // being rendered was built before they were created. Re-reading the
      // order shows them without the user reloading the browser.
      refreshSavedOrder(response?.sampleOrderItems?.labNo);
    } else {
