...
describe("Manage Method Page Tests", () => {
  ...
  it("should open and close the add method modal", () => {
    ...
    cy.get('[data-cy="add-method-modal"]').should("be.visible");
    cy.get('[data-cy="cancel-button"]').click();
    cy.get('[data-cy="add-method-modal"]').should("not.exist");
  });

  it("should show error when adding method with empty fields", () => {
    TestManagementPage.openAddMethodModal();
    TestManagementPage.submitForm();
    cy.get("#englishLabel").parent().should("contain", "This field is required");
  ...
  it("should display existing and inactive methods", () => {
    TestManagementPage.getExistingMethods().should("have.length.greaterThan", 0);
    TestManagementPage.getInactiveMethods().should("have.length.greaterThan", 0);
  });
});
...
describe("Test Catalog Page E2E Tests", () => {
  ...
  it("should filter data correctly when selecting a specific section", () => {
    ...
    TestManagementPage.getSelectedTags().should("contain.text", "Specific Section");
  ...
  it("should display breadcrumb navigation correctly", () => {
    ...
    cy.get(".breadcrumb").contains("breadcrumbs.admin.management").should("exist");
    cy.get(".breadcrumb").contains("sidenav.label.admin.testmgt.ViewtestCatalog").should("exist");
  });
});
