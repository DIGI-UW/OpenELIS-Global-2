...
#
# Loads (in order):
#   1. src/test/resources/e2e-foundational-data.sql
#   2. src/test/resources/analyzer-test-data.sql
#
# Prerequisites:
...
while getopts "f:h" opt; do
  case $opt in
    ...
    h)
      ...
      echo ""
      echo "Loads same fixtures as CI (frontend-qa): e2e-foundational-data.sql, analyzer-test-data.sql"
      exit 0
      ;;
    *) exit 1 ;;
  esac
...
docker compose -f "$COMPOSE_FILE" exec -T "$SVC" psql $PSQL_OPTS < src/test/resources/analyzer-test-data.sql
