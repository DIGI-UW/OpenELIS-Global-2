...
1. Check SystemConfiguration for timeout:
   ```sql
   SELECT * FROM system_configuration WHERE name = 'analyzer.query.timeout.minutes';
   ```
2. If missing, add default (5 minutes):
   ```sql
   INSERT INTO system_configuration (name, value, description)
   VALUES ('analyzer.query.timeout.minutes', '5', 'Query timeout in minutes for analyzer field queries');
   ```
...
- ✅ Query uses timeout from SystemConfiguration
- ✅ If SystemConfiguration missing, defaults to 5 minutes
- ✅ Timeout error displayed gracefully
- ✅ Job status shows "failed" state with error message

**Verification Checklist**:

- [ ] Reads timeout from SystemConfiguration (default: 5 minutes if
      missing/invalid)
