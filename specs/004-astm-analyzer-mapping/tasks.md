...
- [ ] T151f [P] Create connection monitoring service for periodic connection
      tests - File:
...
- [ ] T151g [P] Create error monitoring service to check for unacknowledged
      errors - File:
...
- [x] T151h [P] Refactor AnalyzerLifecycleScheduler to work with unified
      ...
      tests pass.
- [ ] T152a [P] [US1] Update AnalyzersList component to use unified status
      field - File:
...
- [ ] T152b [P] [US1] Update AnalyzerForm component to use unified status
      dropdown - File:
...
- [ ] T152c [P] [US1] Update backend API to use unified status field - File:
...
- [ ] T152d [P] [US1] Update internationalization keys for unified status -
...
- [ ] T152e [US1] Update FieldMapping component to check unified status instead
...
- [ ] T153a [P] [US1] Add unit tests for AnalyzerStatusTransitionService - File:
...
- [ ] T153b [P] [US1] Add unit tests for status event listeners - File:
...
- [ ] T153c [P] [US1] Add integration test for status transitions - File:
...
- [ ] T105 Create AnalyzerQueryServiceImpl in
      ...
      (must pass all before marking complete): - [ ] Implementation connects to
      analyzer via TCP using IP:Port from AnalyzerConfiguration - [ ] Sends ASTM
      LIS2-A2 query message (ENQ/ACK handshake, header frame, EOT) - [ ]
      Receives and parses ASTM response frames
      (STX/FN/data/ETX/checksum/CR/LF) - [ ] Extracts field identifiers from R
      (Result) records per FR-002 requirement - [ ] Parses field metadata:
      fieldName, astmRef, fieldType, unit (handles R-record format:
      `R|seq|astm_ref|field_name||unit|||field_type`) - [ ] Robust parsing
      handles units with special characters (e.g., `10^3/μL`) and composite
      delimiters per research.md Section 14 - [ ] Field type validation against
      AnalyzerField.FieldType enum (defaults to NUMERIC if invalid) - [ ] Stores
      extracted fields in AnalyzerField entity via AnalyzerFieldService - [ ]
      Background job executes asynchronously (returns job ID immediately) - [ ]
      Job status updates progress (0% → 100%) with connection logs - [ ] Reads
      ...
      error message) - [ ] Integration test (T103) verifies actual ASTM query
      execution with mock server - [ ] Manual test: Query analyzer 1000 (mock
      server) and verify fields appear in UI

      **STATUS**: Implementation file exists but was previously a stub. Full implementation
      created 2025-11-26. Verification pending.
