...
- [ ] T069 [US1] Create Liquibase changeset `src/main/resources/liquibase/analyzer/004-008-add-menu-items.xml` for "Analyzers" parent menu item and sub-navigation items ("Analyzers List", "Error Dashboard", "Field Mappings") per FR-020 - Insert into menu table with parent_id, presentation_order, element_id, action_url, display_key, role-based visibility - "Field Mappings" menu item MUST have conditional visibility based on route (contextual - only visible when on `/analyzers/:id/mappings` route) - Backend menu API MUST include contextual items based on route parameters, frontend MUST check current route to determine which contextual items to display per FR-020 specification
...
- [ ] T071 [P] [US2] Integration test for mapping update workflow in `src/test/java/org/openelisglobal/analyzer/service/AnalyzerFieldMappingServiceIntegrationTest.java` using @SpringBootTest - Test methods: `testUpdateMapping_WithExistingResults_PreservesHistoricalData`, `testActivateMapping_WithConfirmation_AppliesToNewMessages`
- [ ] T072 [P] [US2] Frontend unit test for mapping update workflow in `frontend/src/components/analyzers/FieldMapping/MappingPanel.test.jsx` - Test methods: `testUpdateMapping_ShowsConfirmationModal`, `testSaveDraftMapping_DoesNotRequireConfirmation`
- [ ] T073 [P] [US2] Cypress E2E test in `frontend/cypress/e2e/analyzerMaintenance.cy.js` - Test scenarios: "should update existing mapping", "should activate draft mapping with confirmation", "should deactivate mapping while preserving history"

### Implementation for User Story 2

- [ ] T074 [US2] Extend AnalyzerFieldMappingServiceImpl with update methods supporting draft/active workflow per FR-010 - Validate required mappings before activation - Apply changes to new messages only (existing results unchanged)
- [ ] T075 [US2] Add audit trail logging to AnalyzerFieldMappingServiceImpl for all mapping changes (who, when, previous vs new values) per FR-009 - Use BaseObject audit fields (sys_user_id, last_updated)
...
- [ ] T165 [P] [US2] Frontend unit test for MappingActivationModal in `frontend/src/components/analyzers/FieldMapping/MappingActivationModal.test.jsx` - Test methods: `testDisplayModal_ShowsWarningMessages`, `testActivate_WithoutCheckbox_DisablesButton`, `testActivate_WithCheckbox_EnablesButton`
