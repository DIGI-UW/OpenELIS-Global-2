...
- [ ] T021 [P] Create QCResultDAO interface and implementation in `src/main/java/org/openelisglobal/qc/dao/QCResultDAO.java` and `QCResultDAOImpl.java` with methods: findByControlLot, findHistoricalForRule, findByInstrumentAndDateRange
...
- [ ] T024 [P] Create QCRuleViolationDAO interface and implementation in `src/main/java/org/openelisglobal/qc/dao/QCRuleViolationDAO.java` and `QCRuleViolationDAOImpl.java` with methods: findByInstrument, findUnresolved, findBySeverity
...
- [ ] T047 [US6] Create QCControlLotService interface and implementation in `src/main/java/org/openelisglobal/qc/service/QCControlLotService.java` and `QCControlLotServiceImpl.java` with @Service and @Transactional - Methods: insert, update, deactivate, findByInstrumentAndTest, validateLotDates
...
## Phase 4: User Story 8 - Integrate with Instrument Interfaces (Priority: P1)

**Goal**: Enable automatic QC result capture from analyzer ASTM interface to eliminate manual entry and enable real-time quality control evaluation.

**Why Next**: Automated data capture is foundational for operational efficiency and enables all other QC features to work automatically.

**Independent Test**: Simulate ASTM message with QC result, verify automatic mapping to control lot, verify result saved and rule evaluation triggered.

### Tests for User Story 8 (MANDATORY - TDD Enforcement)

- [ ] T056 [P] [US8] Unit test for QCResultService in `src/test/java/org/openelisglobal/qc/service/QCResultServiceTest.java` (Template: `.specify/templates/testing/JUnit4ServiceTest.java.template`) - Test result insertion with auto-evaluation event publishing, test z-score calculation, test manual entry fallback, test validation (result within range) - **Test Slicing**: Use `@RunWith(MockitoJUnitRunner.class)` - **Mocking**: Use `@Mock` for DAOs and event publisher
- [ ] T057 [P] [US8] DAO test for QCResultDAO in `src/test/java/org/openelisglobal/qc/dao/QCResultDAOTest.java` (Template: `.specify/templates/testing/DataJpaTestDao.java.template`) - Test findByControlLot, test findHistoricalForRule (chronological order), test findByInstrumentAndDateRange, test out-of-order result handling - Use `TestEntityManager` for test data
- [ ] T058 [P] [US8] Controller test for QCResultRestController in `src/test/java/org/openelisglobal/qc/controller/QCResultRestControllerTest.java` (Template: `.specify/templates/testing/WebMvcTestController.java.template`) - Test POST /rest/qc/results (manual entry), test POST /rest/qc/results/astm (ASTM capture), test validation errors, test mapping failures - Use `@MockBean` for services, `MockMvc` for HTTP testing
- [ ] T059 [P] [US8] Integration test for ASTM result capture workflow in `src/test/java/org/openelisglobal/qc/QCResultCaptureIntegrationTest.java` (@SpringBootTest with @Transactional) - Test full workflow: ASTM message → result saved → event published → rule evaluation triggered - Verify async event processing completes - Use builders for test data

### Implementation for User Story 8

- [ ] T060 [US8] Create QCResultService interface and implementation in `src/main/java/org/openelisglobal/qc/service/QCResultService.java` and `QCResultServiceImpl.java` with @Service and @Transactional - Methods: insert (with event publishing), calculateZScore, validateResult, findByControlLot - Inject ApplicationEventPublisher - Publish QCResultCreatedEvent after insert
- [ ] T061 [US8] Create QCResultCreatedEvent in `src/main/java/org/openelisglobal/qc/event/QCResultCreatedEvent.java` with resultId payload
- [ ] T062 [US8] Create QCResultForm DTO in `src/main/java/org/openelisglobal/qc/form/QCResultForm.java` with validation annotations (value, unit, controlLotId, instrumentId, testId required)
- [ ] T063 [US8] Create QCResultRestController in `src/main/java/org/openelisglobal/qc/controller/QCResultRestController.java` extending BaseRestController with @RestController and @RequestMapping("/rest/qc/results") - Endpoints: POST / (manual entry), POST /astm (ASTM capture with mapping), GET /by-control-lot/{lotId} - NO @Transactional (belongs in service layer) - Services compile all data within transaction (depends on T060)
- [ ] T064 [US8] Add ASTM result mapping logic to QCResultRestController - Map ASTM fields (instrument ID, test code, control level) to internal entities - Queue unmapped results for manual review - Notify staff of mapping failures
- [ ] T065 [US8] Run formatting: `mvn spotless:apply` for backend (MANDATORY before commit)

**Checkpoint Validation**: At this point, User Story 8 should be fully functional. QC results can be captured automatically from ASTM interface or entered manually. Results are saved with z-scores calculated. Event is published for rule evaluation (evaluation implementation comes in next phase). ALL tests from T056-T059 MUST pass.
...
**Independent Test**: Configure rule set for test instrument, enable/disable specific rules, verify only enabled rules are marked for evaluation.
...
- [ ] T068 [P] [US5] Controller test for WestgardRuleConfigRestController in `src/test/java/org/openelisglobal/qc/controller/WestgardRuleConfigRestControllerTest.java` - Test GET /rest/qc/rule-config/{instrumentId}, test PUT /rest/qc/rule-config (update), test validation errors - Use `@WebMvcTest`, `@MockBean`, `MockMvc`
...
- [ ] T071 [US5] Create WestgardRuleConfigService interface and implementation in `src/main/java/org/openelisglobal/qc/service/WestgardRuleConfigService.java` and `WestgardRuleConfigServiceImpl.java` with @Service and @Transactional - Methods: findByInstrument, updateRuleConfig, applyPreset, validateRuleConfig
...
- [ ] T075 [US5] Add rule configuration route to `frontend/src/App.js`: /analyzers/qc/rule-config/:instrumentId
- [ ] T076 [US5] Run formatting: `mvn spotless:apply` for backend, `npm run format` for frontend (MANDATORY before commit)

**Checkpoint Validation**: At this point, User Story 5 should be fully functional. Lab managers can configure which rules are enabled per instrument, select presets, and validation prevents invalid configurations. ALL tests from T066-T070 MUST pass.
...
- [ ] T102 [P] [US3] DAO test for QCRuleViolationDAO in `src/test/java/org/openelisglobal/qc/dao/QCRuleViolationDAOTest.java` - Test findByInstrument, test findUnresolved, test findBySeverity, test violation queries with filtering - Use `@DataJpaTest` and `TestEntityManager`
...
- [ ] T106 [P] [US3] Cypress E2E test in `frontend/cypress/e2e/qc/qcAlerts.cy.js` - Test AS-001 (1₃ₛ violation triggers immediate email), test AS-002 (multiple 1₂ₛ violations batched), test AS-003 (system notification appears in feed), test AS-004 (click notification - navigate to instrument detail) - Use data-testid selectors, cy.intercept() for email verification

### Implementation for User Story 3

- [ ] T107 [US3] Create QCRuleViolationService interface and implementation in `src/main/java/org/openelisglobal/qc/service/QCRuleViolationService.java` and `QCRuleViolationServiceImpl.java` with @Service and @Transactional - Methods: createViolation(RuleEvaluationResult), findByInstrument, findUnresolved, resolveViolation - Inject QCAlertService to trigger alerts after violation creation
- [ ] T108 [US3] Create QCAlertService interface and implementation in `src/main/java/org/openelisglobal/qc/service/QCAlertService.java` and `QCAlertServiceImpl.java` with @Service and @Transactional - Methods: sendAlertForViolation (with batching logic), sendImmediateAlert (for 1₃ₛ), sendBatchedAlert, determineRecipients - Inject EmailNotificationService for email sending - Implement batching: max 1 per instrument per 15 min, except 1₃ₛ immediate (FR-072, FR-073) (depends on T107)
...
## Phase 8: User Story 1 - View Instrument Compliance Status (Priority: P1) 🎯 MVP

**Goal**: Enable laboratory technicians to see a clear compliance overview with visual indicators so they can quickly assess which instruments require attention.

**Why MVP**: This is the foundational user-facing capability that provides immediate value by showing compliance at a glance.

**Independent Test**: Display dashboard with multiple instruments showing color-coded status (green/yellow/red) based on QC data, and verify technicians can identify non-compliant instruments within 10 seconds.

### Tests for User Story 1 (MANDATORY - TDD Enforcement)

- [ ] T115 [P] [US1] Unit test for QCDashboardService in `src/test/java/org/openelisglobal/qc/service/QCDashboardServiceTest.java` - Test compliance status calculation (green if no violations, yellow if warnings, red if rejections), test instrument card data compilation (within transaction), test last result timestamp tracking - Use `@RunWith(MockitoJUnitRunner.class)`, `@Mock` for DAOs
- [ ] T116 [P] [US1] Controller test for QCRestController in `src/test/java/org/openelisglobal/qc/controller/QCRestControllerTest.java` - Test GET /rest/qc/dashboard (all instruments), test GET /rest/qc/dashboard/{instrumentId} (single instrument detail), test data compilation (no lazy loading), test response format - Use `@WebMvcTest`, `@MockBean`, `MockMvc`
- [ ] T117 [P] [US1] Frontend unit test for QCDashboard in `frontend/src/components/qc/dashboard/QCDashboard.test.jsx` - Test dashboard rendering, test instrument card display, test auto-refresh (5 min interval), test loading states, test empty state (no instruments) - Use `userEvent` and `waitFor`
...
- [ ] T119 [P] [US1] Cypress E2E test in `frontend/cypress/e2e/qc/qcDashboard.cy.js` - Test AS-001 (view dashboard with color-coded instrument cards), test AS-002 (instrument card shows status, triggered rules, last result time), test AS-003 (click card - navigate to detail view) - Use data-testid selectors, cy.request() for test data setup, cy.session() for login
...
- [ ] T123 [US1] Create QCDashboard React component in `frontend/src/components/qc/dashboard/QCDashboard.jsx` using Carbon Grid, Column - Display ComplianceStatusTile for each instrument - Add AlertFeed in top-right corner - Use SWR for data fetching with 5-minute refresh interval (FR-050) - Display last update timestamp - Use React Intl for all strings (depends on T121, T122)
...
**Checkpoint Validation**: At this point, User Story 1 should be fully functional and testable independently. Technicians can view compliance dashboard, identify non-compliant instruments by color within 10 seconds, and navigate to detail views. Dashboard auto-refreshes every 5 minutes. ALL tests from T115-T119 MUST pass. **THIS IS THE MVP - CAN DEPLOY/DEMO NOW.**
...
**Independent Test**: Display Levey-Jennings chart for single instrument with plotted QC results, SD lines (±1SD, ±2SD, ±3SD), and highlighted violation points.

### Tests for User Story 2 (MANDATORY - TDD Enforcement)

- [ ] T127 [P] [US2] Controller test for QCChartDataRestController in `src/test/java/org/openelisglobal/qc/controller/QCChartDataRestControllerTest.java` - Test GET /rest/qc/charts/{instrumentId} (chart data with filtering), test date range filtering, test control level filtering, test data compilation (all relationships fetched) - Use `@WebMvcTest`, `@MockBean`, `MockMvc`
...
- [ ] T131 [US2] Create QCChartDataRestController in `src/main/java/org/openelisglobal/qc/controller/QCChartDataRestController.java` extending BaseRestController with @RestController and @RequestMapping("/rest/qc/charts") - Endpoints: GET /{instrumentId} (with date range, control level filters), GET /{instrumentId}/statistics (mean, SD for reference lines) - Use JOIN FETCH to compile all data (results + violations + statistics) - NO @Transactional - Services compile data (depends on T120 QCDashboardService for data compilation pattern)
...
- [ ] T134 [US2] Add control chart route to `frontend/src/App.js`: /analyzers/qc/charts/:instrumentId
- [ ] T135 [US2] Update ComplianceStatusTile click handler to navigate to /analyzers/qc/charts/:instrumentId (link dashboard to chart detail view)
...
**Independent Test**: Generate trend reports for date range showing violation frequency by instrument, rule type, and severity, with filtering and export capabilities.
...
- [ ] T153 [P] [US7] Controller test for QCTrendAnalysisRestController in `src/test/java/org/openelisglobal/qc/controller/QCTrendAnalysisRestControllerTest.java` - Test GET /rest/qc/trends/compliance (compliance over time), test GET /rest/qc/trends/violations (violation distribution), test filtering (date range, instrument, test, rule type, severity) - Use `@WebMvcTest`, `@MockBean`, `MockMvc`
...
- [ ] T156 [P] [US7] Cypress E2E test in `frontend/cypress/e2e/qc/trendAnalysis.cy.js` - Test AS-001 (view compliance percentage over time with date range filter), test AS-002 (view violation frequency distribution by rule type), test AS-003 (export compliance report - PDF download within 30s), test AS-004 (compare multiple instruments - identify highest violation rates) - Use data-testid selectors, cy.intercept() for export

### Implementation for User Story 7

- [ ] T157 [US7] Create QCTrendAnalysisService interface and implementation in `src/main/java/org/openelisglobal/qc/service/QCTrendAnalysisService.java` and `QCTrendAnalysisServiceImpl.java` with @Service - Methods: calculateComplianceTrend (compliance % over time), getViolationFrequencyDistribution (by rule type, instrument, time period), identifyRecurringViolations (configurable thresholds), calculateMeanTimeToResolution - Use complex HQL queries with aggregations and date grouping
- [ ] T158 [US7] Create QCTrendAnalysisRestController in `src/main/java/org/openelisglobal/qc/controller/QCTrendAnalysisRestController.java` extending BaseRestController with @RestController and @RequestMapping("/rest/qc/trends") - Endpoints: GET /compliance (with date range, instrument, test filters), GET /violations (violation distribution), GET /recurring (instruments with recurring violations), GET /export (PDF report generation) - NO @Transactional (depends on T157)
- [ ] T159 [US7] Create ComplianceTrendChart React component in `frontend/src/components/qc/trends/ComplianceTrendChart.jsx` using @carbon/charts-react LineChart - Display compliance percentage over time, date range filters, instrument/test filters - Use SWR for data fetching - Use React Intl for all strings (depends on T158)
- [ ] T160 [US7] Create ViolationDistributionChart React component in `frontend/src/components/qc/trends/ViolationDistributionChart.jsx` using @carbon/charts-react BarChart - Display violation frequency by rule type, instrument comparison, severity breakdown - Use React Intl for all strings (depends on T158)
...
- [ ] T169 Performance optimization: Add database indexes for QC result queries - Index on (control_lot_id, run_date_time) in qc_result table - Index on (instrument_id, created_at) in qc_rule_violation table - Update Liquibase changeset
...
- [ ] T171 [P] Add monitoring metrics for QC evaluation performance - Instrument rule evaluation duration (track p95 latency) - Alert generation latency (track time from violation to email sent) - Dashboard data retrieval time - Add logging to QC services for performance tracking
...
- **Rule Evaluators (Phase 6)**: Depends on US5, US8 - CRITICAL, enables US1, US2, US3, US4
...
4. Phase 4: User Story 8 (ASTM Interface - automated result capture)
...
- **US8 (ASTM Interface)**: Depends on US6 (control lots must exist for result mapping)
- **US5 (Rule Config)**: Independent - Can start after Foundational
- **Rule Evaluators**: Depend on US5, US8 (need config + results)
...
4. Complete Phase 4: User Story 8 (ASTM Interface - automated capture)
...
9. **STOP and VALIDATE**: Test User Story 1 independently - technicians can view compliance dashboard, identify non-compliant instruments, receive alerts
...
3. Add US8 (ASTM Interface) → Test independently → Automated QC result capture working
4. Add US5 (Rule Config) → Test independently → Lab can configure rules per instrument
...
3. Developer B: User Story 8 (ASTM Interface) - can start after US6 entities created
4. Developer C: User Story 5 (Rule Configuration) - independent
5. After US5, US8 complete: Developer A: Rule Evaluators (critical path)
...
- **MVP Strategy**: Implement critical path (US6 → US8 → US5 → Rules → US3 → US1) to deliver value quickly
...
- Phase 4 (US8 - ASTM Interface): 10 tasks
