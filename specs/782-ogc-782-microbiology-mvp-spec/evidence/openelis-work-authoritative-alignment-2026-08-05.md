...
| 11. M-05        | Enter and interpret AST                                               | MVP M5 plus M8 and T243-T250           | Multi-reading AST, selectable standards, repeat/retest, override/revert history, and review exist; setup confirms and snapshots the panel, standard, exact ordered drug membership, and technique; entry is restricted to that retained order                                                                                       | Partial; justified individual add/drop, complete-set report reconciliation, analyzer/QC review, repeat scope, exact-SHA interaction evidence, and human UAT remain |
...
| M-05 AST                | Manual readings, breakpoint interpretation, selectable and version-snapshotted standard, organism-default panel confirmation, immutable ordered-drug snapshot, reasoned panel adjustment, distinct laboratory technique with server-derived measurement type, reading source/matched-basis/units, no-breakpoint guidance, immutable override/revert history, review, repeat/retest runs, reagent capture                                                  | No individual add/drop interaction or complete-set review/report reconciliation; no `RESULTS_IN`/QC/analyzer metadata path; no accept blockers; no single-drug repeat/NCE convergence                                                                                     | Preserve the delivered manual AST, technique, and ordered-set slices; add the remaining analyzer/QC, ordered-work, and scoped-repeat slices explicitly |
...
- **Migration:** `072-microbiology-ast-provenance.xml` adds only the immutable
  ...
  the only available historical source. All five migrations have explicit
  rollback harnesses and pass update/rollback execution against PostgreSQL.
- **Automated checks passed locally:** focused `MicroAstServiceTest`,
  ...
  reference-data integration tests; all five AST PostgreSQL migration
...
- **Still open:** `1a198a765` snapshots exact panel membership, preserves it on
  repeats, rejects readings outside it, and restricts the Carbon entry control.
  T244 remains open for justified individual drug add/drop and a complete-set
  review/report gate. The repo has no authoritative Antibiotic-to-Test mapping,
  so creating one core `Analysis` per drug would be unsupported; the linked
  culture `Analysis` remains the standard patient-report anchor.
