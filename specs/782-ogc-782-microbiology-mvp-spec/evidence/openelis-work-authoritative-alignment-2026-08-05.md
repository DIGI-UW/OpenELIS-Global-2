...
| M-NFR                   | Exact-SHA service-created qualification separately measures Culture/AST server fetch, browser-ready render, filters, and dense Case rendering. Both worklist grains and filters pass at substantially above-source accumulated row counts, and the Case Timeline defaults to the newest 30 events with full history on demand. At `62b31281a`, the accessibility run passes 16/16 entries: auth setup, seven desktop axe journeys, one desktop full-keyboard journey, and seven mobile axe journeys. M-07 summary cards are canonical keyboard links; route-selected M-04 sections receive focus and a polite status announcement; the keyboard journey reaches preliminary and amendment release; and M-03 plus six touched reference surfaces have compact, focusable table behavior. The five-entry M-03 browser file also passes. | Dense Case p95 is `1,032.7 ms`, so the strict `<1,000 ms` gate remains failed. The source combines 80-100 AST results with only 30 Timeline events while requiring AST edits in the automatic Timeline; the fixture correctly preserves 171 audit events. No offline queue/conflict resolution exists. Direct interaction coverage remains open for the named M-04/M-05 action variants and reference-modal focus return, as do deployed and human accessibility acceptance. The shared URL-less shell still logs a known 404. | Keep T263, T264, and T266 open. Profile the Case without weakening audit history; finish the direct action matrix; diagnose the shared-shell 404 separately; and never infer full WCAG or human acceptance from axe alone. |
...
- **Exact run:** `npm run pw:test:microbiology-accessibility -- --workers=1`
  passed 16/16 entries in 1.4 minutes on `62b31281a`: authenticated setup,
