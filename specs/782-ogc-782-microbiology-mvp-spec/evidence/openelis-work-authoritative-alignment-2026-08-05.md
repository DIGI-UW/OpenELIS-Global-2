...
- **Case stress evidence:** the service-created case contains 5 isolates, 80 AST
  ...
  test and improved dense Case p95 from `1,159.2 ms` to `1,032.7 ms`.
  `77a8b3abe` additionally stops mounting hidden case-section bodies. The
  measurement is retained as regression evidence, not a failed product gate.
- **Requirement-health ruling:** the `<1,000 ms` value entered the authored
  M-NFR on May 27, 2026 and is not present in the workflow mock. It does not
  define runtime mode, network, measurement boundary, or reproducible hardware,
  so it is an engineering baseline leaked into a product artifact. M-04 also
  requires AST edits to appear automatically in
  ...
  rejected. Further optimization is warranted only if representative deployed
  use is observably degraded or a reproducible engineering baseline is adopted.
