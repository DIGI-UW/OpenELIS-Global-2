...
| M-NFR                   | Focused accessibility, keyboard, repeat/amendment, and synthetic performance specs exist. Exact-SHA service-created qualification now separately measures Culture/AST server fetch, browser-ready render, filters, and dense Case rendering. Both worklist grains and filters pass at substantially above-source accumulated row counts. The Case Timeline defaults to the newest 30 events with full history available on demand, as M-04 requires.                                                                                                                                                                                                                                                                                                                                                                                  | Dense Case p95 is `1,032.7 ms`, so the strict `<1,000 ms` gate remains failed. M-04 requires automatic Timeline projection of AST edits while M-NFR asks for 80-100 results with only 30 timeline events; the service-created case correctly preserves 171 automatic events rather than suppressing audit history. No offline queue/conflict resolution exists, and accessibility/audit evidence remains incomplete across new actions. | Keep T263 open for profiling and a production-like exact-SHA pass; record the source-volume contradiction explicitly rather than weakening audit or rounding the result.              |
