...
| 6. M-03         | Derive visible Program and collect culture details during order entry | FR-001/FR-002; Phase 14                | Supported order entry preserves workflow/Method metadata, derives Program, renders the ruled controls, confirms discard, and supports safe manual fallback. Typed booleans, bounds, default-Method resolution, a durable Sample-owned draft across the enter/collect reload, order-search projection, and service-created routing/detail/idempotency integration exist. Patient Origin is still a hardcoded client list and Clinical History is not macro-enabled.                                                                                                                                                                  | Partial; supported-route Playwright proof, deployment reference data, Macro consumer integration, and UAT remain       |
...
| Culture Method is required/defaulted/adjustable                    | FR-002                        | T211/T216/T218      | The shared Method control is required and routing resolves the existing default `TestMethod` before legacy fallback  | Component/service evidence passed; complete supported-save browser proof pending |
...
| Number of Sets uses ruled bounds/default                           | FR-002                        | T211/T216/T218      | UI and service enforce 1-10; persistence integration retains the typed value                                         | Focused automated evidence passed; complete save-path browser proof pending      |
...
| Antibiotic Exposure is a checkbox                                  | FR-002                        | T211/T216/T218      | UI and persistence use binary checkbox/boolean semantics                                                              | Focused automated evidence passed; complete save-path browser proof pending      |
| Critical Notify is a checkbox with ruled default                   | FR-002                        | T211/T216/T218      | UI and persistence use binary checkbox/boolean semantics                                                              | Focused automated evidence passed; specimen-sensitive default proof remains open |
...
| Saving exposes the details on the created case without duplication | US1 scenario 5; FR-001/FR-002 | T212/T218           | Service-created integration data proves Sample-owned draft persistence, order-search projection, routing consumption after reload, case compilation, and repeated-route idempotency; the full `persistData`/REST save path is not yet exercised as one integration test | Partial; complete save-path integration and Playwright remain open               |
