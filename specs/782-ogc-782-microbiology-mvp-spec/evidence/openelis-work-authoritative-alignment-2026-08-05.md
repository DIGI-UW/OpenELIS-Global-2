...
| 11. M-05        | Enter and interpret AST                                               | MVP M5 plus M8 and T243-T250           | Multi-reading AST, selectable standards, repeat/retest, override/revert history, and review exist; setup confirms and snapshots the organism default panel and standard, adjusted panels require a retained reason, readings expose provenance, and laboratory technique now derives MIC/zone measurement type on the server        | Partial; ordered-analysis reconciliation, analyzer/QC review, repeat scope, exact-SHA interaction evidence, and human UAT remain |
...
- **Migration:** `072-microbiology-ast-provenance.xml` adds only the immutable
  ...
  values, and constrains future values. All four migrations have explicit
...
- **Automated checks passed locally:** focused `MicroAstServiceTest`,
  ...
  reference-data integration tests; all four AST PostgreSQL migration
  update/rollback tests; 23 microbiology frontend files with 96 tests; targeted
