...
| 8. M-07         | Work from the shared Culture/AST queue                                | MVP M6 and worklist remediation        | Canonical Culture/AST grain and status state, bounded accession/patient/specimen/panel/activity context, active AST-run projection, pending-setup isolates, Carbon grain switch/cards/tables, exact isolate/run navigation, sibling context, critical flags, complete row menus, positive-signal progression/emphasis, analyzer resistance strip, recent activity, stable 30-second refresh, full-row pointer/keyboard navigation, and repository-role access enforcement exist | Partial; manual resistance confirmation, Method-bound incubation timing/day fallback, scale/visual Playwright, and UAT remain |
...
| M-07 Worklist           | Shared Culture/AST page, canonical grain/status plus shared URL state, bounded accession/patient/specimen/panel/latest-activity projection, active-run and pending-setup projection, Carbon status cards and source-aligned grain tables, exact isolate/run navigation, automatic-results guidance, distinct positive-signal progression and refreshed-row emphasis, analyzer positive-event reconciliation, case-scoped positive/no-growth commands, complete row menus, full-row pointer/keyboard navigation, preserved repeat routing, sibling marker, critical flag, analyzer resistance strip, recent activity, responsive table, stable 30-second/manual refresh, elapsed freshness status, and repository-role worklist access | Manual resistance confirmation is undefined. Day-aware incubation due action/null fallback remains blocked on structured Method-bound culture timing; the current free-text incubation default is not safe to parse. Deployed visual evidence and UAT remain open.        | T257-T258, T277, and T280-T281 complete the page, foundation, semantics, and evidence                                    |
...
- **Still open:** the source does not define how a user manually confirms a
  ...
  and Carbon interaction evidence. T280-T281 retain the incubation day/fallback
  behavior and its missing structured Method-bound timing foundation rather
  than parsing free text or inventing worklist configuration. The 200-row/mobile
