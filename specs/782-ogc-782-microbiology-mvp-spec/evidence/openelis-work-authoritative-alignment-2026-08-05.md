...
| M-07 Worklist           | Shared Culture/AST page, canonical grain/status plus shared URL state, active-run and pending-setup projection, Carbon status cards and grain-specific tables, exact isolate/run navigation, automatic-results guidance, distinct positive-signal progression, analyzer positive-event reconciliation, case-scoped positive/no-growth commands, Culture row menus, partial AST row menus, sibling marker, critical flag, responsive table                                                                             | AST "Set up new AST run" is absent; rows still lack lab/accession, patient, specimen, panel-name, and last-activity display; resistance strip, recent activity, refresh preservation, permission qualification, deployed visual evidence, and UAT remain open             | T248/T254 add the preserved repeat entry point; T276 adds bounded context; T255-T258 complete the page and evidence      |
...
- **Automated checks passed locally:** focused `MicroWorklistServiceTest`,
  ...
  latest focused frontend checkpoint contains 45 passing tests plus targeted
...
- **Material drift still open rather than hidden:** current rows expose internal
  SampleItem/panel identifiers instead of the source's lab/accession, patient,
  specimen, panel-name, and last-activity context. T276 remains the bounded-
  enrichment prerequisite.
- **Still open:** the AST-row "Set up new AST run" preserved repeat entry point,
  resistance hits, recent activity, refresh/focus/scroll preservation, case-
  view permission qualification, 200-row and mobile evidence, registered
  Playwright, separate Grist Culture and AST stories, exact-SHA deployment, and
  human UAT.
