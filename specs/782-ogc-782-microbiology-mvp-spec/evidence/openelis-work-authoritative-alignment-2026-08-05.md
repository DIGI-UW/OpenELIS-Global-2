...
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| M-03 Order Entry        | The supported `/order/enter` path derives and displays Program, persists Method and the ruled culture details, selects Patient Origin from deployment reference data, confirms detail discard, excludes non-culture orders, and creates bacteriology/TB sibling cases on one specimen. The service-created browser fixture has no SQL, fixed primary keys, or DAO bypass and is selected deterministically by external patient ID. All five registered order-entry tests pass, including authentication setup, and the order details pass axe at desktop and mobile sizes.                                                                                                                                                                                                                                                            | Macro Library consumption waits on its separate feature. Source-mock screenshots, live separated Grist publication, exact-SHA AMR deployment, and human acceptance remain open. The direct Java `SamplePatientEntryService.persistData` integration named by T212 remains unproven even though Playwright exercises the real supported save/collect route.                                                        | Keep the macro dependency in T285 and the remaining visual, UAT, deployment, and lower-level integration evidence in T212/T222/T226/T230; do not represent the supported browser proof as deployed acceptance.                               |
...
| M-NFR                   | Exact-SHA service-created qualification separately measures Culture/AST server fetch, browser-ready render, filters, and dense Case rendering. Both worklist grains and filters pass at substantially above-source accumulated row counts, and the Case Timeline defaults to the newest 30 events with full history on demand. At `d39deca1e`, the accessibility run passes 16/16 entries in 1.3 minutes: auth setup, seven desktop axe journeys, one desktop full-keyboard journey, and seven mobile axe journeys. Direct interactions cover canonical routing, all named M-04/M-05 action variants, status announcements, stable Carbon modal lifecycle, and exact focus return for reference editors. The focused modal suite passes 15/15 and fails 3/3 new focus guards when the implementation is inverted.                     | Dense Case p95 is `1,032.7 ms`, so the strict `<1,000 ms` gate remains failed. The source combines 80-100 AST results with only 30 Timeline events while requiring AST edits in the automatic Timeline; the fixture correctly preserves 171 audit events. No offline queue/conflict resolution exists. Deployed and human accessibility acceptance remain open. The shared URL-less shell still logs a known 404. | T264 automated interaction qualification is complete. Keep T263 and T266 open; profile the Case without weakening audit history, diagnose the shared-shell 404 separately, and never infer deployed WCAG or human acceptance from axe alone. |
...
## R1 M-NFR Scale Evidence

- **Method and revision:** exact SHA `8557df5c2`, two warmups plus ten measured
  samples per operation, nearest-rank-ceiling p95, guarded disposable
  PostgreSQL/OpenELIS stack, and service-created fixtures only. Server
  fetch/parse and browser-ready timings are recorded separately.
- **Worklists pass:** with 1,535 active Culture rows and 704 active AST rows in
  the accumulated stress database, Culture server/browser p95 is
  `67.4/1,112.8 ms` and AST server/browser p95 is `70.9/1,144.2 ms`, below the
  `<2,000 ms` source ceiling. Carbon summary-filter p95 is `186.6 ms` for
  Culture and `235.0 ms` for AST, below `<300 ms`.
- **Case remains failed:** the service-created case contains 5 isolates, 80 AST
  readings, and 171 immutable automatic Timeline events. The missing M-04
  recent-30/Show-all behavior was implemented with a focused Carbon interaction
  test and improved dense Case p95 from `1,159.2 ms` to `1,032.7 ms`. The
  strict source gate is `<1,000 ms`, so T263 remains open by `32.7 ms`; no
  rounding or threshold relaxation is permitted.
- **Source contradiction:** M-04 requires AST edits to appear automatically in
  Timeline, but M-NFR describes 80-100 AST results with only 30 Timeline
  events. The fixture preserves the audit projection and the UI bounds its
  default view; deleting or suppressing audit events to manufacture 30 rows is
  rejected. A production-like exact-SHA profile/run must determine the final
  remediation.

## R1 M-NFR Accessibility Evidence - 2026-08-06

- **Authority:** OpenELIS Work M-NFR NFR-04 requires keyboard access to all
  Worklist and Case actions, visible focus, programmatic labels and status,
  focus movement plus an `aria-live` announcement for inline expansion, and
  labelled/focusable data tables. M-07 separately requires its cards and rows
  to be keyboard operable. These observable behaviors govern this audit; axe
  is one automated signal, not the acceptance authority.
- **Primary workflow closure:** `80c36f6b4` changes the M-07 status summary
  cards from pointer-only tiles into canonical links that retain SPA
  navigation, moves focus into the case section selected by the canonical
...
- **Order and reference closure:** `62b31281a` reuses one service-created M-03
  ...
  Carbon button, the mobile order workflow contains and wraps its commands and
  result table, and the touched Patient Origin, organism, antibiotic, AST
  panel, culture Method, and breakpoint tables are keyboard focusable and
  scroll internally instead of expanding the viewport.
- **AST interaction closure:** `cde68ea33` covers analyzer-option expansion,
  override-history reveal and exact revert focus, single-antibiotic repeat scope,
  and preliminary/final report-readiness commands through Carbon keyboard
  interactions. Twenty-five focused tests and the registered keyboard journey
  pass without arbitrary waits.
- **Reference-modal closure:** `d39deca1e` keeps Carbon modal shells mounted,
  keys fresh editor forms, removes synchronous URL-to-draft effects, focuses the
  real AST-panel and breakpoint-import controls, and returns focus to the exact
  generic-reference, AST-panel, or breakpoint command after Escape or successful
  save. Fifteen focused tests pass. Reversing only the implementation makes the
  three new focus tests fail and reproduces Carbon's unmounted-state warning;
  restoring it returns 15/15 green.
- **Exact run:** `npm run pw:test:microbiology-accessibility -- --workers=1`
  passed 16/16 entries in 1.3 minutes on `d39deca1e`: authenticated setup,
  seven desktop axe journeys, one desktop full-keyboard journey, and seven
  mobile axe journeys. The accessibility file covers Worklist/Case, isolate,
  AST, critical communication, reporting, reagent selection, amendment,
  M-03 order details, six reference surfaces, and WHONET. The complete
  registered M-03 file separately passed 5/5 entries in 21.6 seconds. Neither
  run uses arbitrary waits.
- **Defects found by interaction, not styling inference:** the audit exposed
  nondeterministic patient selection, a command represented as an href-less
  link, mobile order-result overlap/page overflow, and an axe
  ...
  test-only timing.
- **Acceptance boundary:** automated T264 is complete. Exact AMR deployment and
