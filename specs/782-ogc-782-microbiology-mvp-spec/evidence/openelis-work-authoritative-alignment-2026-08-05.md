...
| 6. M-03         | Derive visible Program and collect culture details during order entry | FR-001/FR-002; Phase 14                | Supported order entry preserves workflow/Method metadata, derives Program, renders the ruled controls, confirms discard, and supports safe manual fallback. Typed booleans, bounds, default-Method resolution, and service-created routing/detail/idempotency integration exist. Patient Origin is still a hardcoded client list and Clinical History is not macro-enabled.                                                                                                                                                                  | Partial; complete supported-save/Playwright proof, deployment reference data, Macro consumer integration, and UAT remain       |
...
| Saving exposes the details on the created case without duplication | US1 scenario 5; FR-001/FR-002 | T212/T218           | Service-created integration data proves routing/detail persistence, compilation, and repeated-route idempotency; the full supported save/controller path is not yet exercised | Partial; complete save-path integration and Playwright remain open               |
