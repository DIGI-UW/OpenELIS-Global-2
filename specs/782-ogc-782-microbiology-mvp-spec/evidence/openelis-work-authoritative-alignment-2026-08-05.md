...
**Remediation code through:** `8e3337870`
**Remediation branch:**
...
| 11. M-05        | Enter and interpret AST                                               | MVP M5 plus M8 and T243-T250           | Multi-reading AST, selectable standards, repeat/retest, override/revert history, and review exist; setup confirms the organism default panel and snapshots panel/standard versions, adjusted panels require a retained reason, and readings expose source, match basis, units, and no-breakpoint guidance                           | Partial; laboratory technique remains conflated with MIC/ZONE measurement mode; analyzer/QC review, repeat scope, exact-SHA interaction evidence, and human UAT remain |
...
| ----------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| M-04 Case Workbench     | Case creation, deterministic current-step focus, progress rail, compact and expanded Case Info, last-activity context, sticky stage action, typed inoculation/subculture lineage, automatic timeline plus shared Note observations, two-pass isolate work-up and identification, amendments, report release, final lock, workflow classification, sibling navigation, critical entry points, shared NCE reporting, NCE badge, and lost-specimen rejection | No analyzer-event reconciliation; exact-SHA NCE/lost runtime, source-mock comparison, separated Grist stories, and visual/human UAT remain open                                                                                                                           | Keep each remaining behavior in independent TDD/UAT stories; implemented slices are not accepted until deployed UAT                                    |
| M-05 AST                | Manual readings, breakpoint interpretation, selectable and version-snapshotted standard, organism-default panel confirmation, panel-version snapshot, reasoned adjustment, reading source/matched-basis/units, no-breakpoint guidance, immutable override/revert history, review, repeat/retest runs, reagent capture                                                                                                                                     | No standard ordered-analysis projection/report-set reconciliation; no `RESULTS_IN`/QC/analyzer metadata path; no accept blockers; no single-drug repeat/NCE convergence; laboratory technique is still conflated with MIC/ZONE measurement mode                           | Preserve manual AST as delivered subset; separate technique from measurement mode and add the remaining analyzer/QC and ordered-work slices explicitly |
...
- **Migration:** `071-microbiology-isolate-workup.xml` contains only the durable
  ...
  update/rollback remains blocked locally because no Docker daemon is available.
...
- **Migration:** `072-microbiology-ast-provenance.xml` adds only the immutable
  ...
  override/revert event record. All three migrations have explicit rollback
  harnesses. Their XML and standalone update/rollback harnesses compile; runtime
  execution is blocked locally because no Docker daemon is available.
- **Automated checks passed locally:** focused `MicroAstServiceTest` and
  `MicroAstRestControllerTest`; 23 microbiology frontend files with 96 tests;
  targeted ESLint, Prettier, Spotless, Liquibase XML validation, contract YAML
  and reference validation, and `git diff --check`.
- **Still open:** the current code does not create or reconcile a standard
  ordered-analysis projection for the adjusted panel, so T244 remains open.
  ...
  matches. It does not yet represent laboratory technique separately from the
  MIC/ZONE measurement mode, so T243 remains open. `30d8e988a` adds immutable
  ...
  Analyzer/QC lifecycle, exact-SHA Playwright, separated Grist stories,
  deployment, visual comparison, and human UAT remain open.
