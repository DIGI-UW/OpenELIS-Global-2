...
| 8. M-07         | Work from the shared Culture/AST queue                                | MVP M6 and worklist remediation        | Canonical Culture/AST grain and status state, active AST-run projection, pending-setup isolates, Carbon grain switch/cards/tables, exact isolate/run navigation, sibling context, and critical flags exist                                                                                                                          | Partial; positive lifecycle, authoritative row context, menus, folded panels, refresh/permission qualification, Playwright, and UAT remain |
...
| 11. M-05        | Enter and interpret AST                                               | MVP M5 plus M8 and T243-T250           | Multi-reading AST, selectable standards, scoped repeat/retest, NCE Retest convergence, truthful run/setup/identification progress, override/revert history, ordered panel/drug snapshots, justified adjustment, analyzer pending/results/QC states, provenance, review blockers, and durable event reconciliation are implemented   | Partial; worklist repeat origin, exact-SHA interaction evidence, deployment, and human UAT remain                                          |
...
| M-05 AST                | Manual readings, breakpoint interpretation, selectable and version-snapshotted standard, organism-default panel confirmation, immutable ordered-drug snapshot, reasoned panel adjustment, distinct laboratory technique with server-derived measurement type, reading source/matched-basis/units, no-breakpoint guidance, immutable override/revert history, analyzer result/QC lifecycle and provenance, review blockers, whole-panel/single-drug repeat, NCE Retest convergence, truthful progress, reagent capture | No worklist repeat origin, deployed exact-SHA interaction evidence, or human acceptance                                                                                                                                                                                                                 | Preserve the delivered AST slices; complete worklist convergence and acceptance evidence explicitly                                   |
| M-07 Worklist           | Shared Culture/AST page, canonical grain/status plus shared URL state, active-run and pending-setup projection, Carbon status cards and grain-specific tables, exact isolate/run navigation, automatic-results guidance, sibling marker, critical flag, responsive table                                                                                                                                                                                                                                              | The case model has no positive-signal stage or positive analyzer path; rows still lack lab/accession, patient, specimen, panel-name, and last-activity display; menus, resistance strip, recent activity, refresh preservation, permission qualification, deployed visual evidence, and UAT remain open | T275 restores truthful positive/no-growth workflow; T276 adds bounded authoritative context; T254-T258 complete the page and evidence |
...
- **Automated checks passed locally:** focused `MicroWorklistServiceTest`; nine
  route-state tests; Carbon worklist interactions; and AST panel focus tests.
  The focused checkpoint contains 36 passing frontend tests, targeted ESLint,
  Spotless, and `git diff --check`. No migration was needed because the slice
  is a read projection and URL/UI state.
- **Material drift discovered rather than hidden:** the implemented case state
  has no `POSITIVE_SIGNAL` stage or positive analyzer event even though M-04
  and M-07 require a separate positive-to-growth transition. The current rows
  also expose internal SampleItem/panel identifiers instead of the source's
  lab/accession, patient, specimen, panel-name, and last-activity context.
  T275 and T276 now make those prerequisites explicit.
- **Still open:** row overflow navigation, resistance hits, recent activity,
  refresh/focus/scroll preservation, case-view permission qualification,
  200-row and mobile evidence, registered Playwright, separate Grist Culture
  and AST stories, exact-SHA deployment, and human UAT.
...
| M-07   | Entity/table projection language and unused assignment column guidance                                | Present one shared state-driven queue with culture and AST views; no default case ownership                         |
...
- **Migration:** `072-microbiology-ast-provenance.xml` adds only the immutable
  ...
  `078-analyzer-event.xml` adds the shared durable, idempotent event envelope.
  All seven migrations have explicit rollback harnesses and pass
  update/rollback/reapply execution against PostgreSQL.
...
- **Ordered-set closure:** `1a198a765` snapshots exact panel membership,
  ...
  **Still open:** worklist repeat origin, exact-SHA Playwright,
  separated Grist stories, deployment, visual comparison, and human UAT.
