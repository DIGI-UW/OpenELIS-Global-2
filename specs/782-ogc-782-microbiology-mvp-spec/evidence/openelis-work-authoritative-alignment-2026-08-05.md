...
| 11. M-05        | Enter and interpret AST                                               | MVP M5 plus M8 and T243-T250            | Multi-reading AST, selectable standards, repeat/retest, override, and review exist; setup now confirms the organism default panel and snapshots panel/standard versions, while adjusted panels require a retained reason | Partial; reading provenance/matched level, method-specific units, revert history, analyzer/QC review, repeat scope, exact-SHA interaction evidence, and human UAT remain |
...
| M-05 AST                | Manual readings, breakpoint interpretation, selectable and version-snapshotted standard, organism-default panel confirmation, panel-version snapshot, reasoned adjustment, overrides, review, repeat/retest runs, reagent capture | No standard ordered-analysis projection/report-set reconciliation; no reading source or matched-level display/revert history; no `RESULTS_IN`/QC/analyzer metadata path; no accept blockers; no single-drug repeat/NCE convergence; method/source distinction and method-specific units are incomplete | Preserve manual AST as delivered subset; add the remaining analyzer/QC, reading-provenance, and ordered-work slices explicitly |
...
- **Migration:** `072-microbiology-ast-provenance.xml` adds only the immutable
  ...
  Its XML and standalone update/rollback harness compile; runtime execution is
...
- **Still open:** the current code does not create or reconcile a standard
  ...
  Reading source/matched-level display, method-specific units, override revert,
  analyzer/QC lifecycle, exact-SHA Playwright, separated Grist stories,
  deployment, visual comparison, and human UAT also remain open.
