...
| 11. M-05        | Enter and interpret AST                                               | MVP M5 plus M8 and T243-T250           | Multi-reading AST, selectable standards, repeat/retest, override, and review exist; setup confirms the organism default panel and snapshots panel/standard versions, adjusted panels require a retained reason, and readings expose source, match basis, units, and no-breakpoint guidance                                          | Partial; laboratory technique remains conflated with MIC/ZONE measurement mode; revert history, analyzer/QC review, repeat scope, exact-SHA interaction evidence, and human UAT remain |
...
- **Migration:** `072-microbiology-ast-provenance.xml` adds only the immutable
  ...
  context. Both migrations have explicit rollback harnesses.
  Their XML and standalone update/rollback harnesses compile; runtime execution is
  blocked locally because no Docker daemon is available.
- **Automated checks passed locally:** focused `MicroAstServiceTest` and
  `MicroAstRestControllerTest`; 23 microbiology frontend files with 95 tests;
...
- **Still open:** the current code does not create or reconcile a standard
  ...
  MIC/ZONE measurement mode, so T243 remains open. Override revert, analyzer/QC
  lifecycle, exact-SHA Playwright, separated Grist stories, deployment, visual
  comparison, and human UAT also remain open.
