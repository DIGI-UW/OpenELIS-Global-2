...
|---|---|---|---|---|---|---|---|
| M04-CP-01 | From the Microbiology worklist, a reviewer can find the deployment-scoped accession and open its Bacteriology case while retaining worklist query state. | Yes | Yes | Partial | R2 | No | Existing worklist tests cover state-preserving links, but the culture-progression journey currently bypasses the worklist. Add an exact worklist-to-case Playwright path. |
| M04-CP-02 | A received case opens on Inoculation/Setup as the current step, never on Timeline or blindly at the top. | Yes | Yes | Yes | R2 | No | Current-step unit and browser coverage exists. The focused journey must assert it against the fresh case fixture. |
| M04-CP-03 | The Inoculation section offers inline `Start inoculation`; saving a primary bottle/plate records its identifier, media, incubation, atmosphere, and eligible reagent lot. | Yes | Yes | Partial | R2 | No | Service and component tests cover the atomic record and lot. Existing browser coverage omits the lot selection. |
| M04-CP-04 | Saving the first inoculation atomically advances `Received` to `Incubating` and creates an automatic Timeline event with the recorded values, actor, and time. | Yes | Yes | Partial | R2 | No | Service tests cover stage/event/actor/lot atomically. Browser coverage asserts stage and values but not actor/time/lot together. |
...
| M04-CP-08 | While a case is Incubating, the Inoculation section visibly offers `Mark positive` and `Mark no growth`. | Yes | Yes | Component | No | No | The stage-gated actions reuse the existing routed confirmation. Focused Carbon interaction coverage passes; exact browser/deployment proof remains open. |
| M04-CP-09 | The Incubating next-step banner explains the positive/no-growth choice and offers `Mark positive` as the primary next action. | Yes | Yes | Component | No | No | The banner now uses source-aligned guidance and a URL-backed Mark positive action. Focused case-view coverage passes; exact browser/deployment proof remains open. |
...
| M04-CP-11 | Confirming positive advances `Incubating` to `Positive signal`, creates an audited automatic event, and keeps positive signal distinct from confirmed growth. | Yes | Yes | Partial | R2 | No | Service tests prove the distinct stages. Add the deployed user-path assertion and next-step guidance. |
| M04-CP-12 | At Positive signal, guidance points to subculture/Gram stain and does not claim that growth is already confirmed. | Yes | Yes | Partial | R2 | No | The message exists; the exact transition journey must assert it after confirmation. |
| M04-CP-13 | All entry actions are inline Carbon interactions, keyboard reachable, focus-managed, and do not rely on arbitrary waits or forced browser actions. | Yes | Yes | Partial | R2 | No | Focus tests exist for forms and routed confirmations. Run the exact journey using Carbon roles/labels and readiness assertions. |
