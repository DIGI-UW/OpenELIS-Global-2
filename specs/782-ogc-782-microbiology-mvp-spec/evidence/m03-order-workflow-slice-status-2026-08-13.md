...
| --- | --- | --- | --- | --- | --- | --- | --- |
| M03-01 | Selecting a typed culture test visibly derives and locks Program = Microbiology. | Yes | Yes | Partial | R2 | No | Component and Playwright cover the initial selection. Add the missing reload assertion and preserve test workflow metadata in the order-search projection. |
...
| M03-04 | Culture Protocol is read-only and shows the selected test's default Method. | Yes | R2 | Partial | R2 | No | Initial selection is covered. Reload currently renders `Not set`; add component/Playwright coverage and preserve linked Method metadata. |
| M03-05 | The protocol display includes available media/incubation context and explains that the bench owns changes. | Yes | Partial | Partial | R2 | No | Name/helper render; source comparison must confirm whether the configured fixture exposes the required summary. |
...
| M03-18 | Saving and reopening the order preserves Program, protocol, and all entered microbiology details. | Yes | Defect | Missing | Defect | No | Manual UAT showed Program = Unassigned and protocol = Not set after reload. Add controller/component/Playwright tests before fixing the projection. |
| M03-19 | Completing collection creates one RECEIVED case per `(SampleItem, workflow type)` and repeated routing is idempotent. | Yes | Yes | Yes | R2 | No | Integration and Playwright coverage; full-suite fixture contracts are currently red. |
...
| M03-23 | Existing cases retain a Method that is later deactivated and visibly identify it as inactive. | Yes | Partial | Partial | R2 | No | Verify the order/case display and add focused coverage before claiming completion. |
| M03-24 | All controls are keyboard reachable and the inline section remains usable at desktop and mobile widths. | Yes | Yes | Partial | R2 | No | Automated axe/responsive evidence exists; rerun on the corrected SHA. |
...
No implementation change is needed for the current reload defect. Confirm this
interpretation before implementing any future same-workflow, multi-protocol UI.

## Current Slice Gate

The next dependent slice must not start until:

1. M03-18 has failing controller/component/Playwright coverage and a passing
   implementation;
2. M03-05 and M03-23 are verified against the deployed response rather than
   inferred from code;
3. the focused M-03 checks pass on the exact R2 head;
4. the deployed SHA and matching UAT story revision are recorded here;
5. human review remains a separate status and may complete in parallel.
