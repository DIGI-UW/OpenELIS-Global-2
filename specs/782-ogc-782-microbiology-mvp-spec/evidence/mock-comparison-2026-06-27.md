...
| AST entry | Inline AST run table with method, reagent lot, breakpoint standard, per-drug MIC/interpretation/source/override rows. | Actual now shows a result table for the recorded reading, inline override controls, reviewed-state locking, and release-readiness feedback, but only one reading path is exercised. | MVP-level parity; reagent lot/breakpoint metadata is future scope. |
