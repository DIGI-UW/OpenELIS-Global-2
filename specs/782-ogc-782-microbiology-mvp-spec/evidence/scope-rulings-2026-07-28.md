...
| Shared-specimen loss      | M-04 requires sibling cases to retain separate clinical histories, but Mark Lost applies to their one shared physical SampleItem.                                     | Proposed R1 default: reject the shared item and nonterminal analyses, close each open sibling with its own lost event, and block the action when any sibling is final/amended. Piotr must confirm this product behavior during R1 clarification/UAT.                                                         | Ruling pending                   |
...
| Worklist parity           | The deployed M-07 worklist has stable navigation/URL state but omits multiple laboratory context fields and operational views shown in the authoritative mock.        | Describe it as a functional core. Complete richer context, Culture/AST grains, resistance context, and recent activity in a dedicated alignment slice. Preserve the authoritative shared queue: do not add owner or technician filters.                                                                      | Partially resolved in R1         |
