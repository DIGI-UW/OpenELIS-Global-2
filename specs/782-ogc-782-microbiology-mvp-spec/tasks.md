...
- [ ] T269 [R1] Run pinned `tools/code-qa` meaningful coverage, spec/code alignment, simplicity, and evidence workflows across the complete remediation diff. Current checkpoint at `d39deca1e` is recorded in `evidence/code-qa-r1-2026-08-06.md`: focused inversion and restored-green proof pass, current spec/code statuses are reconciled, and the latest interaction slice is lean. Keep this final gate open until T263/T265/T266, stable visual evidence, final video/evidence bundle, Grist publication, and exact deployed evidence are complete.
