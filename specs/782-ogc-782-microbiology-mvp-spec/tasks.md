...
- [x] T253 [R1] Build one Carbon worklist with a Cultures/AST segmented control, grain-specific DataTable headers/rows, baseline state-derived due actions, summary filters, and empty states. `bd27e76d9` completes the shared Carbon structure, URL-backed cards, columns, exact-run links, and compact source-aligned layout; `1d838d72a` restores the distinct positive-signal summary and subculture/Gram-stain due action. The audit found that day-aware incubation text and its null fallback were not covered here; T280 now owns that missing source behavior. Visual runtime evidence remains T257.
...
- [ ] T280 [R1] After T281, implement the authoritative incubation due-action mapping from inoculation time and structured protocol timing. When either input is unavailable, render the source-defined stage fallback; do not infer protocol days from free-text incubation instructions.
- [ ] T281 [R1] Complete the Method-bound culture-timing foundation needed by M-01/M-03/M-04/M-07: add validated structured incubation/max-day/subculture timing to the existing repository extension of Method, expose it through the existing admin/service path, and migrate only this new durable state with update/rollback coverage. Reconcile the current free-text defaults without creating a second culture-protocol master or placing timing logic in the worklist.
