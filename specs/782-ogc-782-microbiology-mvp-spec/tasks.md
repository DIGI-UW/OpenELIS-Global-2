...
- [ ] T243 [R1] Add failing service and React tests for ordered-panel provenance, panel/version and standard/version snapshots, method-driven units, and matched-breakpoint display. Ordered-panel and immutable version snapshots are implemented in `92fb708c8`; `b0dd23456` adds persisted reading source/matched basis/units, Carbon display, and explicit no-breakpoint guidance. Keep this task open until laboratory technique (for example disk diffusion or VITEK) is modeled separately from the current MIC/ZONE measurement mode.
