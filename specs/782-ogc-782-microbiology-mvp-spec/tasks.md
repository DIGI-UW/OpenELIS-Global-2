...
- [ ] T244 [R1] Make AST setup confirm the upstream panel by default; require an audited reason for any panel or drug-set adjustment and retain the exact changed order used for entry and reporting. Confirmation-first setup, server enforcement, actor-linked reason, and panel/version snapshots are implemented in `92fb708c8`. `1a198a765` snapshots the ordered antibiotics on each run, preserves that exact set across later panel edits and repeat runs, rejects out-of-order readings on the server, and restricts the Carbon entry control to the snapshot. Keep this task open for justified individual add/drop interaction and complete-set review/report reconciliation; do not manufacture one core `Analysis` per antibiotic because the repository has no authoritative Antibiotic-to-Test mapping. The existing linked culture `Analysis` remains the standard patient-report anchor.
