...
- [x] T223 [R1] Run the focused Vitest suite for modern order state, Program controls, selected-test metadata, and worklist interaction. Four files and 25 tests passed in 1.77 seconds after the final Program-state simplification on 2026-08-06.
- [x] T224 [R1] Run focused JUnit 4 fixture, ORM, and PostgreSQL update/rollback/reapply checks for the order path and schema corrections. Four classes and 12 tests passed in 40.643 seconds on 2026-08-06.
- [x] T225 [R1] Validate and audit the registered Playwright spec, then run the narrow `core-app` journey using locator readiness and web assertions only. The bucket guard and project validator pass; the final source has no fixed waits, forced actions, blind first/last selectors, or direct response-status assertions; all five tests pass in 18.7 seconds.
...
- [x] T228 [R1] Run Spotless, targeted Prettier/strict ESLint, relevant compilation/tests, Liquibase/Hibernate XML validation, and `git diff --check` before commit. All touched frontend files pass with zero lint warnings; Spotless check and XML parsing are clean on 2026-08-06.
...
- [ ] T258 [R1] Publish separate Culture Worklist and AST Worklist Grist stories and verify both appear independently in the live AMR JSON and rendered overlay. Registered service-created Playwright journeys exist in `77410e82b`; Grist publication, exact live revision, and rendered-overlay evidence remain open.
...
- [ ] T282 [R1] On the exact remediation SHA, capture stable desktop and mobile Culture/AST worklist screenshots, compare them to authoritative M-07 states, record only intentional deviations, and attach the contact sheet plus registered test results. Use readiness assertions and deterministic data, never arbitrary waits.
...
- [ ] T263 [R1] Qualify the case and both worklist grains at source scale using service-layer fixtures; record server and browser timings separately. Partial exact-SHA evidence at `8557df5c2`: against an accumulated disposable dataset of 1,535 active Culture rows and 704 active AST rows, Culture server/browser p95 was `67.4/1,112.8 ms`, AST server/browser p95 was `70.9/1,144.2 ms`, Culture filter p95 was `186.6 ms`, and AST filter p95 was `235.0 ms`; all pass their source ceilings. The service-created dense case has 5 isolates, 80 readings, and 171 immutable automatic timeline events. After implementing the source-required recent-30/Show-all Timeline view, dense Case p95 improved from `1,159.2 ms` to `1,032.7 ms` but still fails the strict `<1,000 ms` gate. Do not close T263 until profiling plus a production-like exact-SHA run passes. OpenELIS Work is internally inconsistent here: M-04 requires each AST edit in the automatic Timeline while M-NFR combines 80-100 results with only 30 timeline events; audit events must not be suppressed to manufacture the smaller fixture.
- [ ] T264 [R1] Audit keyboard, focus, labels, status text, contrast, and compact layouts for every touched action with axe plus direct interactions. Partial exact-SHA evidence is recorded: `80c36f6b4` makes M-07 summary cards semantic canonical links, moves focus into a route-selected M-04 section, announces the expanded section through a polite status region, and extends the keyboard-only case journey through preliminary release and amendment release. `62b31281a` makes the supported M-03 order path and six touched reference-administration surfaces compact and keyboard-scrollable at desktop/mobile sizes, selects the service-created patient deterministically by external ID, and removes the href-less command-link pattern. On exact SHA `62b31281a`, the accessibility run passed 16/16 Playwright entries in 1.4 minutes: authenticated setup, seven desktop axe journeys, the desktop full-keyboard journey, and seven mobile axe journeys. All five M-03 order-entry Playwright entries also passed in 21.6 seconds without arbitrary waits. `833a2ac01` moves focus into primary-inoculation, subculture, and Timeline-note inline forms; announces each expansion; restores focus to the exact opener after Escape, cancel, or successful save; keeps Show-all/Show-recent focus stable; and makes the culture media table keyboard-scrollable. `a7a95944b` moves focus to routed NCE/lost and positive/no-growth action headings, avoids parent-section focus races, proves Carbon overflow commands by keyboard, and provisions the required lost-specimen vocabulary idempotently through audited services. The focused suite passes 35 Carbon tests, 8 fixture-service tests, and the complete case-workbench Playwright file 4/4 in 15.5 seconds without arbitrary waits. Keep T264 open until direct keyboard/focus/status interactions cover M-05 analyzer/QC, override/revert, repeat-scope, and report-readiness variants plus touched reference edit-modal focus return. Deployed and human accessibility acceptance remain T272-T273; axe is not a substitute for either.
