...
| `chromium` | default Playwright spec match (`*.spec.ts`) with dependency on `setup` | `playwright/tests/analyzer-list.spec.ts`, `playwright/tests/analyzer-navigation.spec.ts`, `playwright/tests/error-dashboard.spec.ts`, `playwright/tests/navbar.spec.ts`, `playwright/tests/sidenav.spec.ts` |
