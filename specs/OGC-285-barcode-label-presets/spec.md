...
1. **Given** a user adds CBC + Tissue Biopsy tests to an order, **When** they
   ...
   table shows the union of per-sample presets linked to either test,
...
- A test catalog row has no `test_label_preset_link` rows. Order Entry's Sample
  Labels table falls back to the active system per-sample presets (Specimen /
  Block / Slide / Freezer) and shows "system default" source tags.
...
- An order has zero tests selected. Order Labels table still renders (showing
  all active per-order presets); Sample Labels falls back to system per-sample
  presets.
...
- **FR-014 (TL-6)**: System MUST fall back to `label_preset.default_per_sample`
  when an order contains no tests linked to that preset.
