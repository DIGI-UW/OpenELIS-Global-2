...
- **FR-007**: System MUST integrate with existing analyzer plugins (Mindray,
  ...
  plugin JAR model (`plugins/analyzers/`). All new analyzers (M9-M13) MUST
  also be external plugin JARs. See `docs/analyzer.md`.
...
1. **External Plugin Architecture**: All analyzer plugins are external JARs in
   `plugins/analyzers/` (git submodule from I-TECH-UW/openelisglobal-plugins).
   Plugins register via `connect()` method called by PluginLoader at startup.
   NO analyzer code belongs in `src/main/java/`. See `docs/analyzer.md`.
...
> **Architecture rule**: All plugins live in `plugins/analyzers/` as external
> JARs. NO analyzer code in `src/main/java/`. See `docs/analyzer.md`.

**Analyzers requiring new external plugin JARs** (5 total, in `plugins/analyzers/`):
...
- **Plugin Architecture Guide (CRITICAL)**: `docs/analyzer.md` — External plugin
  model documentation. All new analyzers MUST follow this guide.
- **Research Report**:
