...
- ✅ **ASTM Identification**: Tests `isTargetAnalyzer()` with ASTM headers
  containing "START4" or "STAGO"
- ✅ **HL7 Identification**: Tests `isTargetAnalyzer()` with HL7 MSH segments
  containing "STAGO"
- ✅ **Negative Cases**: Tests rejection of non-Stago analyzers (Horiba,
  Mindray)
- ✅ **Edge Cases**: Tests null/empty input handling
- ✅ **Result Detection**: Tests `isAnalyzerResult()` for both ASTM (R segments)
  and HL7 (OBX segments)
- ✅ **Inserter Factory**: Tests `getAnalyzerLineInserter()` returns correct
  type
...
- ✅ **ASTM Parsing**: Tests parsing of valid ASTM messages with coagulation
  results
- ✅ **HL7 Parsing**: Tests parsing of valid HL7 messages with coagulation
  results
...
- ✅ `stago-start4-coagulation.astm` - Complete ASTM LIS2-A2 message with 5
  coagulation tests
- ✅ `stago-start4-coagulation.hl7` - Complete HL7 v2.5 ORU^R01 message with 5
  coagulation tests
...
Integration tests require Spring context and database access. These should be
added in the main OpenELIS project test suite:
...
**Note**: Some tests may fail without Spring context for persistence operations.
The parsing and identification logic is fully tested.
...
These tests should extend `BaseWebContextSensitiveTest` and use the test
fixtures from `src/test/resources/testdata/stago/`.
