...
- **Date:** 2026-06-28
- **Status:** Analyzer QC/config MVP accepted against the build-bound remote UAT
  contract; merge remains subject to required PR checks
...
| ------------------------------- | -------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| M0 - stabilization              | Accepted (MVP) | Routed QC loading, string-safe control-lot IDs, deterministic empty bridge collections, and bridge-owned FILE runtime are covered by focused tests and the final contract review.                                                  |
| M1 - profile verification       | Accepted (MVP) | The shipped-profile catalog and setup action pass remote UAT. Richer design-gallery filters, used-by/source metadata, and list-level attention presentation remain explicitly deferred.                                            |
| M2 - guided setup               | Accepted (MVP) | Inline creation, lab-unit selection, old-route redirect, exact-once profile application, and saved-analyzer connection testing pass through the visible workflow.                                                                  |
| M3 - mappings and result values | Accepted (MVP) | Catalog-bound values, persisted pending-value resolution, and stale-aware verification pass focused, real-DB, reload, and remote UI guards.                                                                                        |
| M4 - analyzer QC                | Accepted (MVP) | Existing QC entities, readiness gates, bridge payload assembly, fingerprint guards, and visible activation readiness pass automated and remote acceptance.                                                                         |
| C4 - remote UAT and evidence    | Accepted       | A fresh UI-only dry run and MP4 pass all eight required Grist steps. The downloaded Markdown/JSON report identifies the application build, harness build, deployment, checklist revision, actual routes, reviewer, and mark times. |

### Final Acceptance Record - 2026-07-25

- The final evidence run uses the live
  [analyzer UAT instance](https://analyzers.openelis-global.org/login) and the
  Grist-owned schema-v2 checklist.
- `AN-QC-001` through `AN-QC-008` are all required and all pass; the report
  records zero failures, stale answers, or untested required steps.
- The non-video and video stories use visible login and application controls.
  They do not use `page.request`, application API assertions, response polling,
  forced Carbon controls, or arbitrary waits.
- The final evidence bundle contains the H.264 MP4, current desktop and mobile
  viewport screenshots, exact build/checklist metadata, and the downloaded
  Markdown/JSON report. The report, not this mutable roadmap, is authoritative
  for the reviewed application and harness SHAs.
- Current screenshots were compared with the pinned `openelis-work` design.
  The MVP retains Carbon's horizontal table behavior on narrow screens and
  improves mobile navigation by collapsing the sidenav. The richer global
  attention summary in the mock remains deferred rather than silently claimed.
- Browser review found no analyzer backend exceptions or HTTP 5xx responses.
  Existing site-wide notification/service-worker 404s and route-change request
  cancellations are recorded as residual platform noise, not hidden from the
  evidence.
...
| ---------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------ |
| Shipped profiles are a lab-facing setup catalog                        | `AnalyzerRestController`, `AnalyzerTypeManagement`                                                    | profile DTO/controller and route component tests                                                         | [`analyzer-profile-mapping.md`](https://github.com/DIGI-UW/openelis-work/blob/2b590bb1d6ccf8a1c8217aecc8eb5662a05e72a7/designs/analyzer-integration/analyzer-profile-mapping.md) | `AN-QC-001`                                                              | Build-bound desktop/mobile screenshots and passed report                                                     |
| One inline profile-driven creation path                                | `AnalyzersList`, `AnalyzerForm`, analyzer routes                                                      | inline setup, redirect, lab-unit, and endpoint/state exact-once tests                                    | same profile-mapping design; progressive wizard presentation deferred                                                                                                            | `AN-QC-002`                                                              | UI-only dry run, screenshot, MP4, and passed report                                                          |
| Deterministic mapping review records who verified what                 | `FieldMapping`, `PendingCodesPanel`, `SetupVerificationPanel`, setup/pending-code services            | real-DB pending-code mapping plus fingerprint, actor/time, audit, and all-input stale guards             | same profile-mapping design and gap analysis                                                                                                                                     | `AN-QC-003`                                                              | Build-bound UAT mark, current-verification screenshot, and report                                            |
