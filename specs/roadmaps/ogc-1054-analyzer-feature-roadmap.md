...
- [plain-language feature map](../OGC-1054-analyzer-qc-config/feature-map.md);
...
4. **Git is provenance.** Superseded implementation directions are removed
   from active documents or reduced to a short non-executable pointer. Git
   history preserves the old wording; active files must not retain a
   contradictory body merely for audit history. Active branches carry one
   descendant roadmap lineage.
...
| Activation candidate       | The exact profile revision, site-binding/recognition fingerprints, generic site-entered values, and lifecycle intent proposed for verification and Bridge synchronization                                        |
...
model to replace. Its curated profile content and proven Bridge/mock flows
establish the two profile jobs below. The current OpenELIS packaging, copied
defaults, fixed-field setup consumer, and materialized registration payload are
migration inputs, not proof of the target ownership boundary. Packaging profile
files through the OpenELIS distribution does not transfer their architectural
ownership to OpenELIS.
...
- the generated, revision-specific description of site values that OpenELIS
  must collect, including generic field keys, input kinds, choices,
  requiredness, conditions, and defaults;
...
- generic site-entered analyzer-instance values, lifecycle intent, and
  synchronization requests to Bridge.

OpenELIS does not own raw analyzer protocol parsing, instrument listeners,
runtime FILE polling, protocol-specific connection logic, transport or role
decision tables, runtime materialization, or a second portable profile
authority. OpenELIS production code does not branch on ASTM, HL7, FILE, TCP,
MLLP, serial, HTTP, analyzer model, or profile identity to decide setup fields,
registration, probing, activation, or outbound routing.
...
1. OpenELIS pins a Bridge profile ID and revision, renders that exact
   revision's Bridge-generated instance-configuration description, and sends
   analyzer identity, lifecycle intent, and generic site-entered values.
   The registration contract does not contain `AnalyzerQcRule`, control lots,
   Westgard configuration, operational-QC state, an OpenELIS-selected protocol,
   or a transport-specific runtime object.
2. Bridge validates the generic values against the exact revision, materializes
   protocol/transport/listener/parser/runtime behavior, and applies the desired
...
| AC-10      | Setup renders only the exact revision's Bridge-generated fields, choices, requirements, and defaults; OpenELIS does not interpret their runtime meaning, and probe failure never silently changes a saved choice. | Partial; current setup still contains OpenELIS-owned protocol/transport decisions and fixed runtime fields.                                                                | M1 + M3 |
...
Before changing production code for each behavior, the executor MUST state the
exact acceptance criterion, owning repository, forbidden behavior, and required
proof, then audit every affected existing test as retain, rewrite, move to the
owning repository, or delete. Existing tests are not authority merely because
they are recent or green. A test that encodes a responsibility forbidden by the
fixed architecture is removed or rewritten before it can contribute evidence.

After the red-green-refactor cycle and before starting another behavior, the
executor MUST run the applicable `digi-uw/code-qa` spec/code alignment,
meaningful-coverage, simplicity/legacy-removal, and cross-repository companion
checks against the actual diff. The behavior remains incomplete when code and
tests agree with each other but disagree with this roadmap, an owning
specification, or a versioned contract. This per-behavior alignment gate is
required in addition to checkpoint CI and pre-merge review.
...
3. `AnalyzerQcRule` is not retained and no row is migrated or preserved. Bridge
   profiles are curated directly from protocol/vendor/capture evidence; valid
   recognition behavior is represented in profile data only when that evidence
   supports it. The OpenELIS entity/table/service/controller/UI, registration
   field, readiness checks, seed data, translations, and runtime callers are
   removed through E0-M4.
4. Activation is evaluated against one immutable candidate and succeeds if and
   ...
   - the analyzer name is nonblank, at least one active lab unit is assigned,
     and Bridge has accepted and materialized the generic site values against
     that exact revision's generated instance-configuration description;
...
3. Use the current GeneXpert ASTM and FluoroCycler profiles as blocking
   compatibility fixtures. Each must preserve its proven communication/runtime
   behavior and default outcomes across OpenELIS setup, Bridge runtime, and mock
   traffic; compatibility does not preserve an OpenELIS field or payload shape.
4. Curate only the priority evidence-backed set needed by the active MVP:
   GeneXpert ASTM, FluoroCycler FILE, and QuantStudio FILE. Exclude every other
   established source file from the published runtime catalog until its own
   later contract, mock, and assembled-flow evidence exists. No 20-profile
   publication or disposition exercise is an E0/M1 gate.
...
profile jobs, GeneXpert/Fluoro compatibility passes, the priority catalog is
explicit and all other source files are excluded, and clean
replacement/removal tests exist. No M1 code is accepted against a second or
...
1. In BR-M1, put the versioned profile catalog and lifecycle around the
   ...
   the catalog source. The remaining source-file inventory is future curation,
   not an M1 publication gate, and an unproven source file is never loaded as a
   runtime profile or served through an OE fallback.
...
5. Persist a selected profile ID/revision plus explicit local site binding and
   generic site-entered instance values. Profile selection MUST fetch the exact
   revision's Bridge-generated field description and defaults; OpenELIS does
   not parse the profile into protocol, transport, role, FILE/network, or other
   runtime fields. A later revision changes no analyzer until the user
   explicitly selects it, re-verifies, and synchronizes. Retain every referenced
   revision and never persist an authoritative copied profile.
...
1. consume the pinned profile revision's Bridge-generated generic setup
   description, complete URL-backed inline setup/Connect, and remove the
   superseded standalone interfaces;
...
5. In BR-M3, derive one generic instance-configuration description from the
   exact pinned revision and use the same description and validator for setup,
   probe, and desired-state registration. OE-M3 renders and retains only those
   described site values; it does not parse the profile into transport enums,
   fixed connection columns, protocol-specific request fields, or conditional
   FILE/network code. Omission or invalidity in the profile is a Bridge contract
   error, not an OpenELIS fallback. Bridge materializes the selected/default
   transport, role, data flow, listener, parser, and runtime settings and returns
   a normalized summary plus the endpoint a lab must configure. When a Two-way
   probe fails and the pinned profile supports Results only, the Bridge
   description/evidence may offer that explicit supported choice; neither side
   silently rewrites the saved value. For a Bridge receiver/listener, the
   description never asks for a per-analyzer listen address or port: Bridge
   derives the advertised endpoint from its profile and live listener
   configuration. Probe the exact draft candidate transiently: a setup
   connection test neither runs desired-state synchronization nor creates,
   replaces, starts, stops, or otherwise mutates the analyzer's active Bridge
   runtime registration. Bridge validates and materializes the candidate through
   the same pinned-profile contract used by registration, then discards that
   transient runtime after the probe.
...
Repository and behavioral gates also prove that OpenELIS setup, activation,
probe, registration, and outbound-order code contains no analyzer
protocol/transport decision table or runtime materialization. A synthetic valid
profile revision can change its described site fields without an OpenELIS
...
| MVP-013 | The exact Bridge profile revision exposes one generated instance-configuration description with stable generic keys, input kinds, requiredness, choices, conditions, and defaults. OpenELIS renders and persists only those described site values without protocol/transport branching or fixed runtime fields. Bridge alone validates and materializes transport, role, data flow, listener, parser, and runtime settings for both transient probe and desired-state registration. Connection testing returns visible structured success, failure, missing-configuration, and timeout evidence plus the endpoint to configure without mutating active runtime state. Connect displays that evidence inline without a synthetic activity log, generic status-only result, or second modal.                                                                                                                                                                                                           | Bridge schema/service + OpenELIS consumer/guard + RTL + UI E2E |
| MVP-014 | Setup offers only choices and fields in the pinned revision's generated description. For receiver/listener behavior, it never requests a per-analyzer listen address or port; Bridge derives the advertised endpoint from its profile/runtime. Profile omission or invalidity is a Bridge contract error. OpenELIS never infers fields, compatibility, role, data flow, or capability from a profile default, profile identity, protocol, transport, or application constant. A failed round-trip is visible and may expose an explicit Bridge-described Results only choice; it never silently rewrites the candidate. A synthetic valid profile can change its described site fields without changing OpenELIS production code.                                                                                                                                                                                                                                                                    | Bridge contract + OpenELIS integration/guard + RTL + UI E2E    |
...
| MVP-016 | Every transition into `ACTIVE` succeeds if and only if one immutable candidate has: an existing active schema-valid pinned profile revision; a nonblank analyzer name; at least one active lab unit; generic site-entered values accepted and materialized by Bridge against that exact revision; every declared test/result row validly bound or explicitly excluded where offered with exactly matching confirmed row IDs; current recognition confirmation for the same revision/fingerprint, including explicit `NONE`; and a Bridge acknowledgment matching analyzer ID, profile ID/revision, and canonical desired-state fingerprint. One service owns every activation/reactivation. Create/update status fields and mapping, configuration, connection-test, error, or offline events cannot set `ACTIVE` or bypass it. Each false predicate produces one visible blocker. Draft edits/sync never mutate the last active candidate. Operational QC and connection-test outcomes never block. | JUnit 4 + Bridge consumer contract + UI E2E                    |
| MVP-017 | Bridge desired-state synchronization is versioned, idempotent, and deterministic for the pinned profile revision and generic site-entered instance values. OpenELIS does not submit a protocol, transport-specific connection, parser setting, source-routing decision, or materialized runtime shape; Bridge derives those from the pin and values and acknowledges the exact materialized candidate. The schema contains no OpenELIS classifier rules, control lots, Westgard configuration, or other operational-QC state, and repeated identical sync produces no behavioral change.                                                                                                                                                                                                                                                                                                                                                                                                             | Cross-repository contract tests + repository guard             |
...
| Cross-repo contracts      | Pinned profile revision, Bridge-generated setup description, generic-value registration without OpenELIS runtime decisions or operational QC, normalized patient/control FHIR, raw unknown/control context, FILE delivery | Versioned fixtures run by both producer and consumer plus a synthetic-profile change |
...
Every behavior slice and the final code-quality gate use `digi-uw/code-qa` for
spec/code alignment, meaningful coverage, simplicity and legacy removal, and
cross-repository companion status. G0 additionally runs its final acceptance
package gate. A checkpoint cannot defer a discovered alignment failure to the
pre-merge or G0 run.
...
| `AN-MVP-008` | Review the Bridge-described site fields, enter values, run the connection test in inline Connect, then bookmark and reload it.                                                                         | The selected revision's fields, choices, requirements, and defaults are clear; Bridge's structured success/failure/missing/timeout evidence and configure endpoint remain visible without protocol/developer fields, a separate modal, or a simulated activity log.                                                                       |
