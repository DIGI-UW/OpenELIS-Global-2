...
| `AMB-M1-003`   | Migration  | `OPEN`     | Current code proves `defaultConfigId` was transient and generic `analyzer_type` is not portable profile identity. Before the migration writer runs, fix the operator recovery contract for selecting a profile revision and preserving unmatched legacy rows in durable anomalies. No filename, copied-JSON, plugin-type, or first-match heuristic is permitted; independent Bridge catalog composition may proceed. |
| `AMB-M1-004`   | Identity   | `OPEN`     | The functional Analyzer Types concept is reusable mappings, while ADR-001 permits multiple OpenELIS site-binding aggregates for one Bridge profile revision. Before the composed list/detail writer or URL contract is implemented, define whether a linkable type ID denotes the Bridge profile, the site-binding aggregate, or an explicit tuple, and how multiple local forks are named and counted. |
