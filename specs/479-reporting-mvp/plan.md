...
- [x] Two functional milestones delivered through the ten-PR stack described
      above. Do not extend the legacy Routine
...
Iteration checks follow the [validation and review policy](uat.md#iteration-validation-and-review-responsibilities).
Run affected automated checks before merge and reuse selected workflows on the
deployed stage. Grist holds concise human review and feedback; operational
qualification runs when its underlying behavior changes.
