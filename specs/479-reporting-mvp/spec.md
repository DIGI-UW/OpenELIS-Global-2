...
- **FR-020**: Prioritize a low-friction functional journey: the built-in picker
  opens with no selected columns, matching the mock, and provides clear groups,
  search and Add controls. An instance-supplied report definition may provide
  useful ordered defaults explicitly. Retain edits, populate supported filters
  from the instance and provide a clear download action. A user who already has
  the necessary OpenELIS access must not complete an additional
  access-configuration or approval workflow to demonstrate reporting.
