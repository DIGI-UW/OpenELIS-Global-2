...
- **Request**: multipart file + `client_source` (MEDICAL_LAB|EXTERNAL_CLIENT|RESEARCH).
...
- **Response**: paged list of assignments with sample metadata and requested tests.
...
- **Body**: `{ status, assigned_to_user_id?, methodology?, reassignment_reason? }`.
...
- **Body**: `{ oos_flag: true, oos_investigation_id, root_cause?, disposition? }`.
...
- **Body**: `{ "notebook_id": "NB-123", "include_samples": ["SMP-1"], "format": "PDF" }`.
...
- **Body**: `{ "recipient": "ACME Pharma", "delivery_method": "EMAIL", "release_notes": "Released to sponsor" }`.

## Post-Test Handling / Retention

### GET `/rest/analytical/retention`
- **Filters**: `status`, `retention_end_before`, `notebook_id`, `sample_item_id`.
...
- **Body**: `{ disposition_status: "DISPOSED"|"EXTENDED", "retention_end_date"?: "YYYY-MM-DD", "notes"?: "text" }`.

## Security & Audit
- All endpoints require RBAC permissions; audit trail logs user, timestamp, before/after values for assignments, results, reports, and retention operations.
- Controllers remain stateless; transactions start in services, eager fetch to avoid lazy load in controllers.
