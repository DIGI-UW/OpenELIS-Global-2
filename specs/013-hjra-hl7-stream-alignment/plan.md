...
- **BS-Series Validation**: No dedicated BS-200 or BS-300 profile JSON exists in
  the repo yet. `mindray-bs360e.json` covers a different model (BS-360E) and may
  be used only as a structural template for creating BS-200/BS-300 profiles, not
  as evidence that those models are already specified.
- **Harness Usage**: Use `projects/analyzer-harness/` for environment alignment.
  When HL7 support is present in the mock, use its HL7 template/profile and
  simulate API (or MLLP listen) so the full stack—mock → bridge →
  `/analyzer/hl7`—is tested with a specific analyzer type. Do not overstate
  current HL7 harness maturity where the visible local setup is more mature for
  ASTM than for MLLP.
