...
- `validation_status` = PASS if `temperature` >= 121°C AND `pressure` >= 15 psi
...
**Total Entities**: 16 core entities + extensions to existing Sample/Patient
entities

**Key Relationships**:

- Hierarchical: StoragePosition (self-referencing), ViralCulture (passage
  tracking)
- One-to-Many: ViralSample → ColdChainLog, PCRRun → PCRResult, ELISARun →
  ELISAResult
...
- All tables include `sys_user_id` (who created) and `lastupdated` (when
  modified)
- Sensitive actions (biosafety approval, result review, lot release) include
  operator/reviewer tracking
