...
**Branch**: `151-storage-location-crud` | **Date**: 2025-12-11 | **Spec**: [spec.md](./spec.md)  
**Input**: Feature specification from `/specs/151-storage-location-crud/spec.md`  
**Jira**: [OGC-68](https://uwdigi.atlassian.net/browse/OGC-68)

## Summary

Implement full CRUD (Create, Read, Update, Delete) capabilities for storage locations (Room, Device, Shelf, Rack) in the Storage Dashboard. Includes adding connectivity configuration fields (`ip_address`, `port`, `communication_protocol`) to `StorageDevice` for future IoT monitoring integration. Strict referential integrity prevents deletion of locations with children or assigned samples.
...
**Constraints**: >70% test coverage per constitution, FHIR R4 integration mandatory  
**Scale/Scope**:
...
- [x] **Carbon Design System**: UI uses @carbon/react exclusively (Modal, TextInput, Select, OverflowMenu)
...
- [x] **Test Coverage**: Unit + integration + E2E tests planned (>80% backend, >70% frontend)
...
- [x] **Security & Compliance**: RBAC (Manage Storage permission), audit trail via BaseObject

**Complexity Justification Required If**: None - fully compliant with constitution.

## Milestone Plan

_GATE: Features >3 days MUST define milestones per Constitution Principle IX._

**Effort Estimate**: ~3-4 days (borderline - using 2 milestones for manageable PR scope per PR #2421 guidance)
...
- **Small Feature Optimization**: Since M1 and M2 are independent, both can target `develop` directly (no feature integration branch needed)
...
- [x] **Controller Tests**: REST API CRUD endpoints (BaseWebContextSensitiveTest)
...
1. **Existing CRUD Patterns**: How are other OpenELIS entities (Patient, Test) managed via CRUD?
2. **Deletion Cascade**: Does Hibernate cascade delete handle referential integrity, or manual validation needed?
3. **IP Validation**: Standard IPv4/IPv6 regex patterns for form validation
4. **Communication Protocols**: What protocols besides BACnet might be supported in future?
...
**Output**: See [contracts/storage-crud-api.json](./contracts/storage-crud-api.json).
...
1. **Shared Modal Pattern**: Single `StorageLocationModal` component handles all entity types (Room, Device, Shelf, Rack) with dynamic field rendering based on `entityType` prop.

2. **Deletion Validation in Service Layer**: Check for children and assignments in `StorageLocationServiceImpl.canDelete(entityType, id)` - returns validation result with error details.

3. **Optimistic UI**: Frontend shows loading state during CRUD operations, refreshes table on success.
