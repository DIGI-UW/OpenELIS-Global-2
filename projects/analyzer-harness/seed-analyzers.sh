...
verify_uat_mtb_result_option() {
  ...
  option_count="$(psql_query "
    SELECT COUNT(*)
      FROM clinlims.analyzer analyzer
      JOIN clinlims.analyzer_test_map mapping
        ON mapping.analyzer_id = analyzer.id
       AND mapping.analyzer_test_name = 'MTB'
      JOIN clinlims.test_result_component component
        ON component.id = COALESCE(
          NULLIF(mapping.component_id, ''),
          (
            SELECT primary_component.id
              FROM clinlims.test_result_component primary_component
             WHERE primary_component.test_id = mapping.test_id
               AND primary_component.is_primary IS TRUE
               AND primary_component.is_active = 'Y'
             LIMIT 1
          )
        )
      JOIN clinlims.test_result result_option
        ON result_option.component_id = component.id
       AND result_option.is_active IS TRUE
      JOIN clinlims.dictionary dictionary
        ON dictionary.id::text = result_option.value
       AND dictionary.is_active = 'Y'
       AND LOWER(dictionary.dict_entry) = LOWER('Detected')
     WHERE analyzer.name = 'Cepheid GeneXpert (ASTM Mode)';
  ")"
