...
**Important:** Use a recoverable script session (e.g., `screen`) when working on
remote servers. Reconnect using `screen -x` if disconnected.
...
4. Ensure sufficient disk space for database file duplication at
...
- Copy entries from `/var/lib/openelis-global/data2/pg_hba.conf` to
