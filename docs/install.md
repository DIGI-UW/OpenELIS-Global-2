...
2. Choose UI language and keyboard layout.
3. Set the network configuration.
4. Use the default mirror.
5. Set up the hard disk.
6. Name the system: openelis
7. User: openelis
...
9. Select Install OpenSSH server.
   - This will allow you to ssh into this computer, allowing copy/paste for
     ...
     the shell in LINUX.
10. Skip the optional server snaps.
11. Finalize the Ubuntu install.
12. Reboot.
...
2.  Open a command prompt and enter the following commands. This will install
...
3.  Install Net Tools in order to find the IP Address:
...
##### Running OpenELIS Global 3.x in Docker
...
The command below is for generating and using a self-signed certificate. **Note:
...
1.  Download OpenELIS Global2 Installer
    ...
    [Download latest installer package Here:](https://www.dropbox.com/sh/47lagjht4ynpcg8/AABORyLmkpVTtRReeD6wSnJra?dl=0)
        ...
        curl -L -O https://url_for_the_file.tar.gz
...
2.  Unpack and enter the installer by running the following commands in
    ...
    a. EG:
        ...
        tar -xvf OpenELIS-Global_2.3.2.2_Installer.tar.gz

    b.

        cd OpenELIS-Global_2.3.2.2_Installer/
...
6. Local FHIR store is the link to the local FHIR API.
7. The remote FHIR store is used in the use case where OpenELIS is polling for
   lab orders and returning results. EG:
   [the FHIR2 Module for OpenMRS](../deployomrs).
8. The Consolidated Server is a central server which collects lab data for
   reporting, serves as a master facility list, etc.

Wait while install procedure completes.

Please note: OpenELIS Global 2.x is designed for and is tested on Chrome only.
