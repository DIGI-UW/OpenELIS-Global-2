# Welcome to The OpenELIS Global 2.0 Documentation

For more OpenELIS Information, please visit
[openelisglobal.org](https://openelis-global.org).

This Documentation is broken down into broad catagories, with links at the top,
information for Software Developers (this is mostly information about how to
write the code for OpenELIS, how to contribute to the code case, ETC) and
Information for those deploying OpenELIS Global (this contains SOPs focused on
how to deploy, installation of software, setup of a bulid server, etc)

## Configuration

- **[Configuration Documentation](configuration/README.md)** - Detailed guides for configuring OpenELIS domains
  - [Roles Configuration](configuration/roles-configuration.md) - User role management
  - [Dictionaries Configuration](configuration/dictionaries-configuration.md) - Dictionary entries for dropdowns
  - [Questionnaires Configuration](configuration/questionnaires-configuration.md) - FHIR questionnaires for data collection

## For Developers

- [Development Setup](dev_setup.md)
- [Development Guide](dev.md)
- [Messages and Internationalization](messages.md)

## For Deployers

- [Installation Guide](install.md)
- [Implementation Guide](implementation.md)
- [Troubleshooting](troubleshooting.md)
- [Password Management](password.md)

## Upgrades

- [Upgrade from 1.x to 2.x](upgrade1to2.md)
- [2.7 Upgrade Guide](27upgrade.md)

## Additional Resources

- [Admin Manual](OEGlobal_AdminManual_2024.md)
- [User Manual](User%20Manual.md)
- [Entity Relationship Diagram](entity%20relation%20diagram.md)
- [Direct FHIR Communication](directFHIRCommunication.md)
- [Deploy with OpenMRS](deployomrs.md)
- [Certificate Setup Report](certificate-setup-report.md)
