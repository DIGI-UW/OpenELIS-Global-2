...
public class FileAnalyzerReaderIntegrationTest extends BaseWebContextSensitiveTest {
    ...
    private void cleanTestData() {
        try {
            // Delete file import configurations for test analyzer
            jdbcTemplate.execute("DELETE FROM file_import_configuration WHERE analyzer_id IN "
            ...
            // Delete analyzer results for test analyzer
            jdbcTemplate.execute("DELETE FROM analyzer_results WHERE analyzer_id IN "
                    + "(SELECT id FROM analyzer WHERE name LIKE 'TEST-%')");
            ...
            System.out.println("Failed to clean file import test data: " + e.getMessage());
        }
    }
