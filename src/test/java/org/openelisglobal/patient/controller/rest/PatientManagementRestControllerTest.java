...
public class PatientManagementRestControllerTest {
    ...
    void getPhoto_withValidRole_returnsPhoto() throws Exception {
    ...
    @Test
    @WithAnonymousUser
    void getPhoto_withoutAuth_returns403() throws Exception {
        mockMvc.perform(get("/rest/patient-photos/1/false"))
    ...
    @Test
    @WithMockUser(roles = "PATIENT_VIEW")
    void getPhoto_withNonNumericId_returns400() throws Exception {
    ...
    @Test
    @WithMockUser(roles = "PATIENT_VIEW")
    void getPhoto_whenPhotoIsNull_returns404() throws Exception {
    ...
    @Test
    @WithMockUser
    void savepatient_whenBindingErrors_doesNotPersist() throws Exception {
        // Sending invalid JSON or incomplete body to trigger validation errors
        // Note: The specific validation details depend on SamplePatientEntryForm.SamplePatientEntry.class
        String invalidJson = "{}"; 

        mockMvc.perform(post("/rest/PatientManagement")
                .contentType(MediaType.APPLICATION_JSON)
                .content(invalidJson))
                .andExpect(status().isBadRequest()); // Or 200 depending on global handler, but check persistence

        verify(patientService, never()).persistPatientData(any(), any(), anyString());
    }
}
