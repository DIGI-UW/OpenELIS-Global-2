...
public class ReportingRecoveryIntegrationTest extends BaseWebContextSensitiveTest {
    ...
    @Test
    @Transactional(propagation = org.springframework.transaction.annotation.Propagation.NOT_SUPPORTED)
    public void missingReadyOutputExpiresTheJobAndReturnsAGoneResponse() throws Exception {
        var job = submit();
        String worker = UUID.randomUUID().toString();
        jobs.claim(worker);
        Files.writeString(files.stage(job.id(), worker), "complete");
        jobs.publish(owner, job.id(), worker, 1);
        Files.delete(files.path(job.id()));

        ReportingException missing = assertThrows(ReportingException.class, () -> jobs.download(owner, job.id()));

        assertEquals(410, missing.status());
        assertEquals("reporting.job.expired", missing.getMessage());
        assertEquals("EXPIRED", jobs.detail(owner, job.id()).state());
    }
