...
public class MicrobiologyUatScenarioServiceTest {
    ...
    public void provisionsAnalyzerAwaitingAstScenarioThroughServices() {
        ...
        when(analyzerService.getMatch("id", "analyzer-1")).thenReturn(java.util.Optional.of(analyzer));
    ...
    @Test
    public void analyzerScenarioRejectsAnUnknownSourceBeforeCreatingClinicalFixtures() {
        MicrobiologyUatScenarioRequestForm request = new MicrobiologyUatScenarioRequestForm();
        request.scenario = "AST_ANALYZER_REVIEW";
        request.scenarioKey = "unknown-analyzer";
        request.analyzerInstrumentId = "unknown";
        when(analyzerService.getMatch("id", "unknown")).thenReturn(java.util.Optional.empty());

        IllegalArgumentException error = assertThrows(IllegalArgumentException.class,
                () -> service.provision(request, "1"));

        assertEquals("AST_ANALYZER_REVIEW requires an existing analyzer", error.getMessage());
        verify(sampleService, never()).getSampleByAccessionNumber(anyString());
    }
