...
public class MicrobiologyTestFixtures {
    ...
    public String firstMethodId() {
        return methodService.getAllActiveMethods().stream().filter(method -> method.getId() != null)
                .sorted(Comparator.comparing(Method::getId)).map(Method::getId).findFirst()
                .orElseGet(this::createMethodId);
    }
