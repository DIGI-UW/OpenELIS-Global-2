...
public class MicrobiologyReferenceDataIntegrationTest extends BaseWebContextSensitiveTest {
    ...
    public void finalReleaseAppearsInTheStandardPatientReportWithReviewedSirResults() throws Exception {
        ...
        MicroIsolate isolate = isolateService.createIsolate(scenario.caseId, "ISO-1", "Gram negative rod",
                "Lactose fermenting", MicroIsolateSignificance.CLINICALLY_SIGNIFICANT, performedBy);
        isolate = isolateService.updateIdentification(isolate.getId(), referenceData.organism().getId(),
                ...
                MicroIsolateIdentificationStatus.CONFIRMED, "MALDI-TOF", new BigDecimal("99.0"), performedBy);
        String reportedIsolate = "ISO-1: " + referenceData.organism().getDisplayName();
        ...
        MicroAstRun run = astService.startRun(isolate.getId(), panelId, standard.getId(),
                "Use the UAT panel for the release journey", performedBy);
        ...
        assertTrue("Projected Result is missing: " + projectedValues,
                projectedValues.stream().anyMatch(value -> value.contains(reportedIsolate)));
        ...
        assertTrue("Projected Result is missing from the standard report aggregation",
                rawPatientReportResults(scenario.patientId).stream().anyMatch(
                        item -> item.getResult() != null && item.getResult().getValue().contains(reportedIsolate)));
        ...
        assertTrue("Expected microbiology content in patient report values: " + reportValues,
                reportValues.stream().anyMatch(value -> value.contains(reportedIsolate)));
