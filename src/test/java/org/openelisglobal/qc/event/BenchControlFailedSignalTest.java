...
public class BenchControlFailedSignalTest extends BaseWebContextSensitiveTest {
    ...
    public void invalidRdtControl_raisesTheSignalWithoutAStatisticalViolation() {
        ...
        NcEvent nce = ncEventService.getMatch(Map.of("triggerSourceType",
                QcViolationNceServiceImpl.TRIGGER_SOURCE_BENCH_CONTROL, "triggerSourceId", failing.getId()))
                .orElse(null);
    ...
    private QCResult persistBenchControl(QCSource source, QCQualitativeOutcome outcome, String testId,
        ...
        BenchQCCaptureFormBuilder capture = BenchQCCaptureFormBuilder.create(source).withTestId(testId)
                .withTestSectionId(LAB_UNIT).withControlLotId(controlLotId).withOutcome(outcome).withResultValue(value)
                .withTarget(new BigDecimal("100.00000"), new BigDecimal("5.00000"))
                .withRunDateTime(runAt.toLocalDateTime());
        if (source == QCSource.RDT) {
            capture.withControlLabel("Malaria RDT · LOT-BENCH-1");
        }
        return qcResultService.createBenchQCResult(capture.build(), TECHNICIAN);
    }
    ...
    private void seedInControlPredecessor(String testId, String controlLotId) {
        qcResultService.createBenchQCResult(BenchQCCaptureFormBuilder.create(QCSource.MANUAL).withTestId(testId)
                .withTestSectionId(LAB_UNIT).withControlLotId(controlLotId).withOutcome(QCQualitativeOutcome.PASS)
                .withResultValue(new BigDecimal("100.00000")).withRunDateTime(LAST_IN_CONTROL.toLocalDateTime())
                .build(), TECHNICIAN);
    }
