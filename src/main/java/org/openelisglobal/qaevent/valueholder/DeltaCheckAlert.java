...
/**
 * Entity class for delta check alerts. Delta check alerts are generated when
 * lab result values change by more than the configured threshold compared to
 * previous results for the same patient.
 */
@Entity
...
public class DeltaCheckAlert extends BaseObject<Integer> {
    ...
    public enum AlertStatus {
        ACTIVE("Alert is active and needs attention"), DISMISSED("Alert has been dismissed by user"),
    ...
    @Basic
    @Column(name = "previous_result_date")
    ...
    public DeltaCheckAlert(String resultId, String previousResultId, BigDecimal currentValue, BigDecimal previousValue,
    ...
    private static BigDecimal calculateChangePercent(BigDecimal current, BigDecimal previous) {
        ...
        BigDecimal percentChange = difference.divide(previous, 4, RoundingMode.HALF_UP)
    ...
    public String toString() {
        return "DeltaCheckAlert{" + "id=" + id + ", resultId=" + resultId + ", previousResultId=" + previousResultId
                + ", currentValue=" + currentValue + ", previousValue=" + previousValue + ", changePercent="
                + changePercent + ", thresholdPercent=" + thresholdPercent + ", status='" + status + '\''
