...
/**
 * In-house blinding backend (OGC-612..08): seal a panel and distribute it as
 * standard OpenELIS orders keyed by blind code, then unblind at the deadline
 * and score every participant result against the sealed targets.
 */
public interface EQABlindingService {
    ...
    /**
     * "Seal panel &amp; distribute": seals the panel (existing validation: targets
     * present, in-house unblind date), then for every panel sample creates a
     * standard order — Sample with the blind code as its accession number (the
     * Workplan handoff), SampleItem, NotStarted Analysis, sample_eqa row linked to
     * the cycle/round — plus a DRAFT eqa_participant_result carrying the analyst
     * assignment, and finally moves the panel to DISTRIBUTED. Every panel sample
     * must appear in {@code specs} exactly once.
     *
    ...
    /**
     * DISTRIBUTED → UNBLINDED (the state machine makes a second call a no-op
     * conflict, which is the idempotency guard), then every participant result in
     * the panel's cycle is resolved: submitted results are scored against the
     * sealed target (numeric acceptance range, else categorical exact match);
     ...
     * Unacceptable verdicts open one Follow-Up Queue row per cycle (never auto-NCE
     * for in-house). The panel ends SCORED.
     */
    EQAPanel unblindAndScore(Long panelId, String sysUserId, EQAUnblindMethod method);

    /**
     * Scores results answered <i>after</i> their panel was unblinded.
     *
     * <p>
