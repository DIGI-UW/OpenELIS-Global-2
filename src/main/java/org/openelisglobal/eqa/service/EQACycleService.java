...
public interface EQACycleService extends BaseObjectService<EQACycle, Long> {
    ...
    /**
     * A new cycle in PLANNED (and the same create the provider wizard needs). A
     * null cycle number takes the scheme's next one, which is what the wizard
     * suggests anyway.
     *
    ...
    /**
     * The same create, carrying the cycle's distribution method. Every HTTP create
     * routes here — an in-house round simply has no method to carry, and passes
     * null.
     */
    EQACycle create(Long schemeId, Integer cycleNumber, String cycleName, Date plannedStartDate, Date plannedEndDate,
    ...
    /**
     * Move a cycle to a new state and record why, atomically. Either the status
     * changes and exactly one audit row appears, or neither does.
     *
     ...
     * than stored. No column backs this.
     */
    ...
    /**
     * Evaluate the ready-to-ship gate once: QC and inventory together, with the
     * per-panel arithmetic the prep workbench displays. The transition enforces
     * exactly this verdict, so the workbench cannot show one gate while another is
     * applied.
     */
    EQAPrepGate evaluatePrepGate(EQACycle cycle);

    /**
     * The laboratories this cycle was created for, as organization ids. Every
     * reader that sizes or addresses a cycle — the prep gate's aliquot arithmetic,
     * the shipment workbench's one-row-per-participant — goes through here, so
     * there is one answer to "who is in this cycle".
     *
     * <p>
     * Cycles created before {@code eqa_cycle_participant} existed have no roster,
     * and fall back to the scheme's active enrollments: the stand-in the shipment
     * workbench used, kept only for those rows so an in-flight cycle does not lose
     * its participants at upgrade.
     */
    List<Long> participantOrganizationIds(EQACycle cycle);

    /**
     * The five-step cycle wizard's single write: cycle, panel, panel samples and
     * participant roster in one transaction, then PLANNED → PREP_IN_PROGRESS.
     * Either the whole cycle exists ready for prep, or none of it does — a
     * half-created cycle would show up on the provider scheme list as a cycle with
     * no panel, which the prep gate then blocks forever.
     *
     ...
     * collected in step 2 with the rest of the panel's source, where the
     ...
     * behind it — the same choice the in-house wizard makes, and the reason
