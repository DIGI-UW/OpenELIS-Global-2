...
public interface EQAPanelService extends BaseObjectService<EQAPanel, Long> {
    ...
    /**
     * A PREPARING panel and its samples in one write. Samples that arrive without a
     * blind code get one derived from the panel id, which is what makes both wizard
     * modes — split an existing pool, or define samples by hand — the same call.
     *
     * <p>
     * Blind codes are filled in only for an IN_HOUSE scheme. A provider panel is
     * shipped as physical material identified by its sample code and is never
     * blinded into local orders, so a blind code on one would assert something
     * untrue about it.
     *
    ...
    /**
     * The display name of an analyte, or null for a null id. A missing row throws:
     * every caller holds an id that a foreign key already vouches for.
     */
    String analyteName(Long analyteId);

    /**
     * PREPARING → SEALED. Refuses a panel with no samples, any sample with a blank
     * target (the encryption converter passes blanks through unencrypted, so blanks
     * must never reach the column), and an in-house panel without an unblind date.
     *
    ...
    /** DISTRIBUTED → UNBLINDED (the reveal point). */
    EQAPanel unblind(Long panelId, String sysUserId);

    /**
     * DISTRIBUTED → UNBLINDED taken under a row lock, recording how the panel was
     * unblinded. The lock is what makes the edge check a real idempotency guard:
     * without it a manual and a scheduled unblind can both read DISTRIBUTED and
     * both proceed to score.
     */
    ...
    /**
     * The panel's samples as DTOs. Sealed-target rule: target value, unit and
     * acceptance range are null unless the panel has reached
