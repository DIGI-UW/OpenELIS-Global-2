...
/**
 * The provider prep and shipment workbenches.
 *
 * <p>
 * Panel material travels as ordinary {@code shipping_box} + {@code shipment}
 * rows — this service is the orchestration over the existing shipment module,
 * not a second shipping implementation. One box per participant per cycle,
 * keyed by a deterministic box id, so re-saving a participant's courier details
 * updates rather than duplicates.
 */
public interface EQAShipmentService {

    /**
     * The provider scheme list: one row per scheme this lab provides — meaning at
     * least one other laboratory is actively enrolled — each carrying its cycles
     * with their participant and panel counts.
     *
     * <p>
     ...
     * roster rows per cycle. This is what replaced the earlier walk over every
     * cycle in the database asking each scheme for its count.
     */
    Map<String, Object> getProviderSchemes();

    /**
     * Prep state for one cycle: participant count, and per panel the produced /
     * reserved / shipped counts against what the inventory rule requires, plus
     * whether the ready-to-ship gate would pass and why not.
     */
    Map<String, Object> getPrepStatus(Long cycleId);

    /**
     * Record prep progress against a panel: aliquot counts and the homogeneity QC
     * verdict. Refuses counts that break the produced >= reserved + shipped
     * invariant before the DB CHECK does, so the workbench gets a message rather
    ...
    /** One row per laboratory on the cycle's participant roster. */
    List<Map<String, Object>> getShipmentRows(Long cycleId);

    /**
     * A partner lab's receipt reached this provider over FHIR (the box's own-store
     * SupplyDelivery reads completed), so record the delivery here. When the box is
     * the participant's current shipment this is exactly the Receipt Monitor's
     * "Mark delivered" — cycle auto-advance and audit included; an outdated box (a
     * repeat superseded it) is closed off quietly without touching the cycle.
     *
    ...
    /**
     * Receipt monitor rows: the shipment rows plus what receiving adds — received
     * date and the overdue verdict. Overdue = not delivered and today is past the
     * expected delivery plus two business days.
     */
    List<Map<String, Object>> getReceiptRows(Long cycleId);

    /**
     * Provider-side manual receipt fallback: the participant lab confirmed arrival
     * out of band, so the provider records the delivery. When the last active
     * participant's box is delivered, the cycle advances shipped → delivered →
     * submissions_open automatically. Idempotent on a delivered box.
     */
    Map<String, Object> markDelivered(Long cycleId, Long organizationId, String sysUserId);

    /**
     * Reprovision a participant: a new shipment recording which one it replaces,
