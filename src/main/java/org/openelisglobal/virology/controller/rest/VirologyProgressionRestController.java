 ...
 * Handles sample advancement from Stage 1 (Reception) → Stage 2 (Culture) →
 * Stage 3 (Vaccine Development)
 */
@Controller
...
public class VirologyProgressionRestController extends BaseRestController {
    ...
    /**
     * Advances samples from one virology workflow stage to the next. Validates
     * stage completion requirements before allowing progression.
     *
    ...
    @PostMapping("/advance")
    @ResponseBody
    public ResponseEntity<?> advanceSamples(@Valid @RequestBody VirologyProgressionForm form, BindingResult result,
            HttpServletRequest request) {

        // Validate form input
        if (result.hasErrors()) {
            String errors = result.getAllErrors().stream().map(error -> error.getDefaultMessage())
                    .reduce((a, b) -> a + ", " + b).orElse("Validation failed");
            return ResponseEntity.badRequest().body("Invalid progression request: " + errors);
        }

        try {
            // Validate stage progression rules
            ValidationResult validation = virologyProgressionService.validateProgression(form.getEntryId(),
                    form.getFromStage(), form.getToStage(), form.getSampleIds());
            ...
            ProgressionResult progressionResult = virologyProgressionService.advanceSamples(form.getEntryId(),
                    form.getFromStage(), form.getToStage(), form.getSampleIds(), getSysUserId(request));

            if (progressionResult.isSuccess()) {
                return ResponseEntity.ok().body("Successfully progressed " + form.getSampleIds().size()
