...
public interface MappingApplicationService {
     ...
     * units, and qualitative values, queries AnalyzerFieldMapping, applies mappings,
     * and returns transformed data structure.
     * 
     * @param analyzerId The analyzer ID
     * @param lines      Raw ASTM message segments (List<String>)
     * @return MappingApplicationResult containing:
     *         - transformedLines: Transformed ASTM lines with mapped values (if
     *         mappings found)
     *         - unmappedFields: List of fields that could not be mapped
     *         - errors: List of errors encountered during mapping
     *         - hasMappings: Whether any mappings were found and applied
     */
