 ...
 * - Type compatibility validation (numeric→numeric, qualitative→qualitative, text→text)
 * - Required mapping validation (Sample ID, Test Code, Result Value)
...
public interface AnalyzerFieldMappingService extends BaseObjectService<AnalyzerFieldMapping, String> {
     ...
     * @param confirmed Whether user confirmed activation (required for active analyzers)
    ...
    /**
     * Get all mappings for an analyzer with complete data compiled
     * Eagerly fetches all relationships within transaction
     * 
     * @param analyzerId The analyzer ID
     * @return List of maps containing complete mapping data (field name, type, etc.)
     */
    List<Map<String, Object>> getMappingsForAnalyzer(String analyzerId);
    
    /**
     * Create a mapping for an analyzer with validation
     * Verifies analyzer field belongs to analyzer, validates type compatibility
     * 
     * @param analyzerId The analyzer ID
     * @param analyzerFieldId The analyzer field ID
     ...
     */
    Map<String, Object> createMappingForAnalyzer(String analyzerId, String analyzerFieldId,
     ...
     * @param confirmed Whether user confirmed the update (required for active analyzers)
     * @return The updated mapping
     * @throws LIMSRuntimeException if confirmation required but not provided, or validation fails
     */
     ...
     * 
     * Sets is_active=false and logs retirement reason for audit trail.
     * Cannot disable required mappings (Sample ID, Test Code, Result Value).
     * 
