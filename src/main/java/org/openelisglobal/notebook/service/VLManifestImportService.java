...
/**
 * Service interface for Virology Laboratory (VL) manifest CSV
 * import.
 *
 ...
 * flexibility.
 */
public interface VLManifestImportService {

    /**
     * Represents a single row from the VL manifest CSV. Each row creates one
     * sample with its associated reception and processing metadata.
     */
    record VLManifestRow(int rowNumber,
            // Required fields
    ...
    record ParsedManifest(List<VLManifestRow> rows, List<ParseError> errors) {
    }

    record VLManifestImportResult(int totalRequested, int totalCreated, List<ParseError> errors,
     ...
     */
    ParsedManifest parseManifestCsv(InputStream csvInput, VLManifestImportForm columnMapping);
     ...
     */
    VLManifestImportResult createSamplesForEntry(Integer entryId, ParsedManifest manifest, String sysUserId);

    /**
     * Get valid sample types for the VL laboratory. Returns sample types that are
     * both in the valid VL list AND exist in the database.
     *
     * @return List of maps containing sample type info (id, description)
     */
    List<Map<String, String>> getValidVLSampleTypes();
}