 ...
 *
 * Branching Workflow: PATH A: Advanced Analysis (Mandatory all 4 steps) - Step
 * 1: Fractionation - Step 2: Identification/Isolation - Step 3: Purification -
 * Step 4: Characterization PATH B: Direct to Production (skips this page)
 *
...
public class TraditionalMedicineAnalyticalServiceImpl implements TraditionalMedicineAnalyticalService {
    ...
    @Override
    @Transactional
    public AnalyticalResponse selectPathway(Integer pageId, PathwaySelectionRequest request, String sysUserId) {
        ...
        for (Integer sampleId : request.sampleIds()) {
            try {
                ...
                log.error("Error selecting pathway for sample {}: {}", sampleId, e.getMessage(), e);
            }
        }

        String message = String.format("Pathway selected for %d sample(s).", updatedCount);
        log.info("Pathway selection complete: pageId={}, pathway={}, updated={}", pageId, pathway.getLabel(),
                updatedCount);
    ...
    @Override
    @Transactional
    public AnalyticalResponse recordFractionation(Integer pageId, FractionationRequest request, String sysUserId) {
        if (request == null || request.sampleIds() == null || request.sampleIds().isEmpty()) {
            return AnalyticalResponse.error("No sample IDs provided");
        }
        ...
        for (Integer sampleId : request.sampleIds()) {
            try {
                ...
                log.info("Fractionation recorded for sample {}: method={}, fractions={}", sampleId, method.getLabel(),
                        request.numberOfFractions());

            } catch (Exception e) {
                log.error("Error recording fractionation for sample {}: {}", sampleId, e.getMessage(), e);
            }
        }
    ...
    @Override
    @Transactional
    public AnalyticalResponse recordIdentification(Integer pageId, IdentificationRequest request, String sysUserId) {
        if (request == null || request.sampleIds() == null || request.sampleIds().isEmpty()) {
            return AnalyticalResponse.error("No sample IDs provided");
        }
        ...
        for (Integer sampleId : request.sampleIds()) {
            try {
                ...
                if (request.activeConstituentsFound() != null && !request.activeConstituentsFound().isBlank()) {
                    data.put("activeConstituentsFound", request.activeConstituentsFound());
                }
                if (request.knownCompoundsIdentified() != null && !request.knownCompoundsIdentified().isBlank()) {
                ...
                log.error("Error recording identification for sample {}: {}", sampleId, e.getMessage(), e);
            }
        }
    ...
    @Override
    @Transactional
    public AnalyticalResponse recordPurification(Integer pageId, PurificationRequest request, String sysUserId) {
        if (request == null || request.sampleIds() == null || request.sampleIds().isEmpty()) {
            return AnalyticalResponse.error("No sample IDs provided");
        }
        ...
        for (Integer sampleId : request.sampleIds()) {
            try {
                ...
                if (request.purityAssessmentMethod() != null && !request.purityAssessmentMethod().isBlank()) {
                    PurityAssessmentMethod assessmentMethod = PurityAssessmentMethod
                            .fromId(request.purityAssessmentMethod());
                ...
                log.info("Purification recorded for sample {}: method={}, purity={}%", sampleId, method.getLabel(),
                        request.purityLevel());

            } catch (Exception e) {
                log.error("Error recording purification for sample {}: {}", sampleId, e.getMessage(), e);
            }
        }
    ...
    @Override
    @Transactional
    public AnalyticalResponse recordCharacterization(Integer pageId, CharacterizationRequest request,
            String sysUserId) {
        if (request == null || request.sampleIds() == null || request.sampleIds().isEmpty()) {
            return AnalyticalResponse.error("No sample IDs provided");
        }

        if (request.spectroscopyTechniques() == null || request.spectroscopyTechniques().isEmpty()) {
        ...
        for (Integer sampleId : request.sampleIds()) {
            try {
                ...
                for (String techId : request.spectroscopyTechniques()) {
                    ...
                    techniqueLabels.add(technique != null ? technique.getLabel() : techId);
                }
                data.put("spectroscopyTechniqueLabels", techniqueLabels);

                if (request.structureDetermination() != null && !request.structureDetermination().isBlank()) {
                    data.put("structureDetermination", request.structureDetermination());
                }
                if (request.propertiesIdentified() != null && !request.propertiesIdentified().isBlank()) {
                    data.put("propertiesIdentified", request.propertiesIdentified());
                }
                if (request.characterizationNotes() != null && !request.characterizationNotes().isBlank()) {
                ...
                log.info("Characterization recorded for sample {}: techniques={}", sampleId,
                        String.join(",", request.spectroscopyTechniques()));

            } catch (Exception e) {
                log.error("Error recording characterization for sample {}: {}", sampleId, e.getMessage(), e);
            }
        }

        String message = String.format("Recorded characterization for %d sample(s).", updatedCount);
    ...
    public Map<Integer, Map<String, Object>> getAnalyticalStatus(Integer pageId) {
        ...
        for (NotebookPageSample nps : samples) {
            ...
            if (nps.getData() != null) {
                ...
                analyticalData.put("fractionationComplete", data.get("fractionationComplete"));
                analyticalData.put("identificationComplete", data.get("identificationComplete"));
                analyticalData.put("purificationComplete", data.get("purificationComplete"));
                analyticalData.put("characterizationComplete", data.get("characterizationComplete"));
                analyticalData.put("chromatographyMethod", data.get("chromatographyMethod"));
                analyticalData.put("chromatographyMethodLabel", data.get("chromatographyMethodLabel"));
                ...
                analyticalData.put("spectroscopyTechniques", data.get("spectroscopyTechniques"));
            }
        ...
        for (Integer sampleId : sampleIds) {
            try {
                ...
                if (!Boolean.TRUE.equals(data.get("characterizationComplete"))) {
                    errors.add(String.format("Sample %d: Characterization not complete", sampleId));
                }
    ...
    @Override
    @Transactional
    public AnalyticalResponse markAnalysisComplete(Integer pageId, List<Integer> sampleIds, String sysUserId) {
        if (sampleIds == null || sampleIds.isEmpty()) {
            return AnalyticalResponse.error("No sample IDs provided");
        }
        ...
        int updatedCount = notebookPageSampleService.bulkUpdateStatus(pageId, sampleIds,
        ...
        log.info("Analysis marked complete: pageId={}, updated={}, routing to next page", pageId, updatedCount);
