...
import org.openelisglobal.notebook.form.VLManifestImportForm;
import org.openelisglobal.notebook.service.VLManifestImportService.VLManifestImportResult;
import org.openelisglobal.notebook.service.VLManifestImportService.VLManifestRow;
import org.openelisglobal.notebook.service.VLManifestImportService.ParseError;
import org.openelisglobal.notebook.service.VLManifestImportService.ParsedManifest;
...
/**
 * Virology Laboratory (VL) manifest import implementation.
 *
 * Parses VL-specific CSV, creates samples + sample items, links to notebook
 ...
 *
 * Sample Types validated against VL-specific list (DNA, RNA, cDNA, genomic
 * materials).
 */
@Service
public class VLManifestImportServiceImpl implements VLManifestImportService {

    private static final DateTimeFormatter DATE_TIME_FORMATTER = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm");
    private static final DateTimeFormatter DATE_FORMATTER = DateTimeFormatter.ofPattern("yyyy-MM-dd");

    /**
     * Valid sample types for the Virology Laboratory. Must match
     * descriptions in type_of_sample table from Liquibase scripts.
     */
    private static final Set<String> VALID_VL_SAMPLE_TYPES = Set.of(
            // DNA samples
    ...
    @Override
    public ParsedManifest parseManifestCsv(InputStream csvInput, VLManifestImportForm columnMapping) {
        List<VLManifestRow> rows = new ArrayList<>();
        ...
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(csvInput, StandardCharsets.UTF_8))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                rows.add(new VLManifestRow(rowNumber, sampleId, sampleType, source, collectionDate, receptionDateTime,
    ...
    public List<ParseError> validateManifest(ParsedManifest manifest) {
        ...
        for (VLManifestRow row : manifest.rows()) {
            ...
            } else if (!VALID_VL_SAMPLE_TYPES.contains(row.sampleType().toLowerCase())) {
                errors.add(new ParseError(row.rowNumber(), "sampleType",
                        "Sample Type '" + row.sampleType() + "' is not valid for VL"));
            }
    ...
    @Override
    @Transactional
    public VLManifestImportResult createSamplesForEntry(Integer entryId, ParsedManifest manifest, String sysUserId) {
        List<ParseError> errors = new ArrayList<>();
        ...
        if (optEntry.isEmpty()) {
            ...
            return new VLManifestImportResult(0, 0, errors, createdSampleIds);
        }
        ...
        return new VLManifestImportResult(totalRequested, totalCreated, errors,
                createdSampleIds.stream().distinct().toList());
    }

    private Map<String, Object> getData(VLManifestRow row, NotebookPageSample nps, Sample parentSample) {
    ...
    @Override
    public List<Map<String, String>> getValidVLSampleTypes() {
        List<Map<String, String>> result = new ArrayList<>();
        for (String sampleType : VALID_VL_SAMPLE_TYPES) {
