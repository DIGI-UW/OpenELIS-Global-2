...
import org.openelisglobal.notebook.service.ViralVaccineManifestImportService;
import org.openelisglobal.notebook.service.ViralVaccineManifestImportService.ParseError;
import org.openelisglobal.notebook.service.ViralVaccineManifestImportService.ParsedManifest;
import org.openelisglobal.notebook.service.ViralVaccineManifestImportService.ViralVaccineManifestImportResult;
import org.openelisglobal.notebook.service.ViralVaccineManifestImportService.ViralVaccineManifestRow;
...
/**
 * REST controller for Viral & Vaccine Unit Laboratory manifest CSV import
 * operations. Handles Viral & Vaccine-specific data points and sample creation.
 */
@RestController
@RequestMapping(value = "/rest/notebook/viralvaccine")
public class ViralVaccineManifestImportController extends BaseRestController {

    @Autowired
    private ViralVaccineManifestImportService viralVaccineManifestImportService;

    @Autowired
    private NotebookEntryService notebookEntryService;

    /**
     * Preview Viral & Vaccine manifest CSV for a notebook entry. POST
     * /rest/notebook/viralvaccine/entry/{entryId}/samples/preview-manifest
     *
     * @param entryId the notebook entry ID
     * @param file    the CSV file
     * @param form    Viral & Vaccine column mapping configuration
    ...
    public ResponseEntity<Map<String, Object>> previewManifestForEntry(@PathVariable("entryId") Integer entryId,
            @RequestPart("file") MultipartFile file, @RequestPart("mapping") ViralVaccineManifestImportForm form) {
        ...
        try (InputStream inputStream = file.getInputStream()) {
            // Parse the CSV
            ParsedManifest parsed = viralVaccineManifestImportService.parseManifestCsv(inputStream, form);

            // Validate sample types
            List<ParseError> validationErrors = viralVaccineManifestImportService.validateSampleTypes(parsed);
            ...
            response.put("totalSamplesToCreate",
                    parsed.rows().stream().mapToInt(ViralVaccineManifestRow::numOfSamples).sum());
            response.put("rows", parsed.rows().stream().map(row -> {
                ...
                rowMap.put("sourceOrganism", row.sourceOrganism());
                ...
                rowMap.put("originLab", row.originLab());
                rowMap.put("storageLocation", row.storageLocation());
                rowMap.put("storageTemperature", row.storageTemperature());
                rowMap.put("volume", row.volume());
                rowMap.put("intendedUse", row.intendedUse());
    ...
    /**
     * Create Viral & Vaccine samples from manifest CSV for a notebook entry. POST
     * /rest/notebook/viralvaccine/entry/{entryId}/samples/create-from-manifest
     *
     * @param entryId     the notebook entry ID
     * @param file        the CSV file
     * @param form        Viral & Vaccine column mapping configuration
    ...
    public ResponseEntity<Map<String, Object>> createSamplesForEntry(@PathVariable("entryId") Integer entryId,
        ...
        try (InputStream inputStream = file.getInputStream()) {
            ...
            ViralVaccineManifestImportResult result = viralVaccineManifestImportService.createSamplesForEntry(entryId,
                    parsed, sysUserId);
