...
/**
 * Exception thrown when a duplicate accession number is detected during sample creation.
 * This can occur when attempting to insert a sample with an accession number that
...
public class DuplicateAccessionNumberException extends RuntimeException {
     ...
     * @param cause the underlying exception (typically a DataIntegrityViolationException)
     */
    public DuplicateAccessionNumberException(String accessionNumber, int attemptNumber,
            int maxAttempts, Throwable cause) {
        super(String.format(
                "Duplicate accession number '%s' detected on attempt %d of %d. %s",
                accessionNumber, attemptNumber, maxAttempts,
