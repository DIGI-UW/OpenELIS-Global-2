...
public class InventoryLotDAOImpl extends BaseDAOImpl<InventoryLot, String> implements InventoryLotDAO {
    ...
    public List<InventoryLot> getAvailableLotsByItemFEFO(String itemId) throws LIMSRuntimeException {
        try {
            ...
            String hql = "FROM InventoryLot l " + "WHERE l.inventoryItem.id = :itemId "
                    + "AND l.status IN (:activeStatuses) " + "AND l.qcStatus = :passedStatus "
                    + "AND l.currentQuantity > 0 "
                    + "ORDER BY l.expirationDate ASC NULLS LAST, l.calculatedExpiryAfterOpening ASC NULLS LAST";

            Query<InventoryLot> query = entityManager.unwrap(Session.class).createQuery(hql, InventoryLot.class);
            query.setParameter("itemId", itemId);
            query.setParameter("activeStatuses", List.of(LotStatus.ACTIVE, LotStatus.IN_USE));
    ...
    public List<InventoryLot> getExpiringLots(int daysAhead) throws LIMSRuntimeException {
        try {
            ...
            String hql = "FROM InventoryLot l " + "WHERE l.status IN (:activeStatuses) "
