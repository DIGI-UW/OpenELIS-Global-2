...
/**
 * Service interface for generating inventory report data Provides data
...
public interface InventoryReportService {
    ...
    /**
     * Generate stock level report data Shows current inventory status for all (or
     * active) items
     *
     ...
     */
    List<StockLevelReportData> generateStockLevelData(boolean includeInactive, boolean groupByType,
            boolean groupByLocation);

    /**
     * Generate low stock alert report data Shows only items below reorder threshold
     *
     * @param includeInactive Include inactive items (default: false)
    ...
    /**
     * Generate expiration forecast report data Shows lots expiring within date
     * range
     *
     ...
     */
    List<ExpirationForecastReportData> generateExpirationForecastData(LocalDate startDate, LocalDate endDate,
            boolean includeExpired);

    /**
     * Generate transaction history report data Shows all inventory transactions
     * within date range
     *
     ...
     */
    List<TransactionHistoryReportData> generateTransactionHistoryData(LocalDate startDate, LocalDate endDate);

    /**
     * Generate usage trends report data Shows consumption patterns and analytics
     * for date range
     *
     ...
     */
    List<UsageTrendReportData> generateUsageTrendData(LocalDate startDate, LocalDate endDate);

    /**
     * Generate lot traceability report data Links specific lots to test results and
     * patient identifiers
     *
     * @param lotNumber Specific lot number to trace (optional - if null, returns
     *                  all lots)
