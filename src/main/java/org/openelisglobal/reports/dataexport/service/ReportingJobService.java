...
public class ReportingJobService {
    ...
    @Transactional(noRollbackFor = ReportingException.class)
    public Download download(String owner, String id) throws java.io.IOException {
        ...
        try {
            input = files.open(id);
        } catch (java.nio.file.NoSuchFileException missingOutput) {
            stored.transitionTo(ExportJobState.EXPIRED);
            ReportingAudit.job(ReportingAudit.Action.EXPIRED, "system", stored);
            throw new ReportingException(410, "reporting.job.expired");
        }
