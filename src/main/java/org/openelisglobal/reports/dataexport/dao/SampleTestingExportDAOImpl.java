...
public class SampleTestingExportDAOImpl extends BaseDAOImpl<Result, String> implements SampleTestingExportDAO {
    ...
    @Override
    public List<TestResultComponent> allComponents() {
        return entityManager.createQuery("from TestResultComponent c order by c.testId, c.displayOrder, c.id",
                TestResultComponent.class).getResultList();
    }

    @Override
    public List<UnitOfMeasure> unitsOfMeasure() {
        return entityManager.createQuery("from UnitOfMeasure u", UnitOfMeasure.class).getResultList();
    }
